...
async function saveScenesToDatabase(movieId: string, scenes: any[]) {
...
async function updateMovieWithAnalysis(movieId: string, overallScaryScore: any) {
  const updates = {
...
export async function POST(request: NextRequest) {
  try {
    ...
    await saveScenesToDatabase(movieId, analysis.scenes);
    console.log('💾 Saved scenes to database');
    
    // Update movie with overall scary score
    if (analysis.overall_scary_score) {
      await updateMovieWithAnalysis(movieId, analysis.overall_scary_score);
