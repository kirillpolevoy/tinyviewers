    ...
    if (jsonMatch) {
      ...
      const normalizedScenes = parsed.scenes.map(scene => ({
        ...
        age_flags: scene.age_flags ? Object.fromEntries(
          Object.entries(scene.age_flags).map(([age, flag]) => [`${age}y`, flag])
        ) : {
          '2y': '⚠️',
          '3y': '⚠️', 
          '4y': '⚠️',
...
async function runClaudeAnalysis(claudePrompt) {
  ...
  try {
    ...
    // Filter for movies that have subtitles but no scenes
...
runClaudeAnalysis(claudePrompt); 