// COPY of experiments/trigger-scan/excerpts.js as of commit 5fe8fb5 — choose the most relevant
// subtitle lines for a flagged beat, and nothing else.
//
// No API calls. Everything comes from the recording (per-beat probabilities, beat cue ranges, which
// beats are flagged) plus the cues parsed from the subtitle file.
//
...
//
// Two differences from the original: `buildExcerpts` takes the recording and the cues as values
// instead of reading recordings/<slug>.jev.json and data/<slug>.srt off disk, and there is no CLI;
// and `verifyExcerpts` at the bottom is the exposure half of verify-excerpts.js, ported so that a
// live run checks its own output against the policy instead of trusting the rule that produced it.
// The keyword tables, the scoring and `chooseExcerpts` are byte-identical, because verify-excerpts.js
// checks the output of this rule and the two must not drift.
import { PRESENCE, EVENTS } from './taxonomy-v3.js';
import { EXCERPT_MAX_LINES, EXCERPT_MAX_WORDS } from './jev-core.js';
...
/**
 * The excerpt file for one recording: `{ "<beat id>": [{ cue, line, score, why }] }`, flagged beats
 * only, in film order — the exact content of recordings/<slug>.excerpts.json.
 *
 * @param {object} recording a recording as `record.js` produces it
 * @param {Array<{id: string, text: string}>} cues the film's cues, from srt.js parseSrt
 */
export function buildExcerpts(recording, cues) {
  const byId = new Map(cues.map((c) => [c.id, c]));
  const top = new Map(recording.thresholds.flagged.map((f) => [f.beat_id, f.top]));

  const out = {};
  for (const beat of recording.beats) {
...
/**
 * The exposure policy, checked on the output rather than trusted from the rule that produced it.
 *
 * PORTED from experiments/trigger-scan/verify-excerpts.js: the same assertions, over values instead
 * of over recordings/<slug>.excerpts.json. It runs on every live film because `jobs.excerpts` and
 * `recordings.excerpts` are two of the four places in this schema that hold subtitle text at all
 * (see the header of schema.sql), and "the rule is careful" is not the same thing as "the output is
 * inside the policy". A bug in `chooseExcerpts` would otherwise quietly widen what we quote.
 *
 * What is checked is exactly what the exception was granted for: flagged beats only, at most two
 * lines a beat, at most twelve words a line, each line really being that cue's own text. The
 * `why`-block assertions stay in the experiment — they guard citation integrity, not exposure, and
 * a wrong citation is not a rights question.
 *
 * The 12-20% band that RECORDINGS.md reports is deliberately NOT a failure here. A short film has
 * few words and the same two lines per flagged beat, so Room on the Broom legitimately sits above
 * the band; `excerptExposure` reports the number and a person reads it.
 *
 * @returns {{ fails: string[] }} every problem, named by beat and cue. No subtitle text in them.
 */
export function verifyExcerpts(excerpts, recording, cues) {
  const byCue = new Map(cues.map((c) => [c.id, c]));
  const beats = new Map(recording.beats.map((b) => [b.id, b]));
  const fails = [];
  const ok = (cond, msg) => { if (!cond) fails.push(msg); };

  const flaggedIds = recording.beats.filter((b) => b.flagged).map((b) => b.id);
  ...
  for (const id of flaggedIds) ok(id in excerpts, `${id}: flagged but has no excerpt`);

  for (const [id, list] of Object.entries(excerpts)) {
    const beat = beats.get(id);
    if (!beat) continue;
    const lo = cueNum(beat.start_cue);
    const hi = cueNum(beat.end_cue);
    // A beat whose cues are not all in the track can only quote the ones that are.
    const beatCues = chooseableCues(byCue, lo, hi);
    ...
    for (const e of list ?? []) {
      ...
      ok(byCue.has(e.cue), `${id}: cue ${e.cue} is not in the subtitle file`);
      ...
      // The quoted line must be that cue's line, whole or truncated with an ellipsis — never
      // anything stitched together across cues or paraphrased.
      const real = byCue.get(e.cue)?.text.replace(/\s+/g, ' ').trim() ?? '';
...
const chooseableCues = (byCue, lo, hi) => {
  let n = 0;
  for (let i = lo; i <= hi; i++) if (byCue.has(`C${String(i).padStart(4, '0')}`)) n += 1;
  return n;
};

/** Words quoted, and how many of the film's own words that is: the exposure number RECORDINGS.md reports. */
export function excerptExposure(excerpts, cues) {
  const lines = Object.values(excerpts).flat();
  return {
    beats: Object.keys(excerpts).length,
    lines: lines.length,
    withEvidence: lines.filter((e) => e.why.items.length).length,
    words: lines.reduce((s, e) => s + wordCount(e.line), 0),
    filmWords: cues.reduce((s, c) => s + wordCount(c.text), 0),
  };
}
