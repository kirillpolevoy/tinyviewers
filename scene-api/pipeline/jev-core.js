// COPY of experiments/trigger-scan/jev-v3-core.js as of commit 5fe8fb5 — the Jev v3 analysis core.
//
...
// passes its own `post`, which is how the recorder times every request without changing the work
// being done.
//
// Two differences from the original, both because this runs inside a request rather than a CLI:
//   1. `analyzeFilm` takes the already-parsed `windows` instead of a slug it would read off disk
//      through common.js `loadFilm`. The subtitles reach a serverless function over the wire; they
//      are never a file here.
//   2. the API key and `fetch` are arguments rather than reads of module-scope globals, so a test
//      can run a whole film against a stub.
// The beats, the 103 questions per beat, the model, the concurrency of 8, the token ceiling and the
// splitting rule are unchanged, because a recording made here has to be the same artefact as one
// made by record-jev-run.js.
import { PRESENCE, EVENTS, MODIFIERS, SEVERITY_SCORES } from './taxonomy-v3.js';
import { pool, postJson } from './net.js';
...
export const authHeaders = (apiKey = process.env.TYPESAFE_API_KEY) => ({ Authorization: `Bearer ${apiKey}` });
...
// ---- the run --------------------------------------------------------------------------------------
// `post(body, ctx)` must behave like net.js postJson: resolve to { json, latencyMs, ... }.
// ctx describes what the request is for, which is all a recorder needs to label it. `ctx.signal` is
// the pool's cancellation: a window failing must stop the other seven, not leave them spending
// against a job that is already lost.
export async function analyzeFilm({ windows: all, limit, concurrency = CONCURRENCY, post, apiKey, fetchImpl } = {}) {
  const headers = authHeaders(apiKey);
  const send = post ?? ((body, ctx) => postJson(API_URL, headers, body, { fetchImpl, signal: ctx?.signal, service: 'jev' }));
  ...
  let splitWindows = 0;
  let maxEst = 0;
  const perWindow = await pool(windows, concurrency, async (w, _i, signal) => {
    ...
    for (const [part_i, part] of plan.entries()) {
      ...
      const res = await send(
        ...
        { kind: 'beats', windowId: w.id, part: part_i, parts: plan.length, estTokens: part.est, questionCount: Object.keys(part.questions).length, beatIndices, signal },
      );
