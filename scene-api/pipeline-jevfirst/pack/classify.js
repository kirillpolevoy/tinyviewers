...
export async function run({ slug, runId = 'r1', cap = 0.25, concurrency = MAX_CONCURRENCY, limit, scenesWanted, dryRun = false, post, segFile }) {
  ...
  // v10.1: credits scenes (credits.js) are not asked; they get a row with skipped 'end_credits' (select.js: no flags)
  const credits = creditsSceneIds(seg.scenes, cues, JSON.parse(fs.readFileSync(path.join(here, 'policy.json'), 'utf8')).credits ?? {});
  const creditRows = scenes.filter((s) => credits.ids.has(s.id)).map((s) => ({ id: s.id, start_cue: s.start_cue, end_cue: s.end_cue, start_ms: s.start_ms, end_ms: s.end_ms, n_lines: s.end_cue - s.start_cue + 1, skipped: 'end_credits', requests: [], answers: null }));
  scenes = scenes.filter((s) => !credits.ids.has(s.id));
  ...
  if (dryRun) return { ...header, dry_run: true, scenes: plans.length, credits_scenes: creditRows.map((r) => r.id), ...estimate, est_tokens_per_scene: Math.round(estimate.est_tokens_total / plans.length) };
  ...
  const out = {
    ...
    credits: credits.span, credits_scenes: creditRows.map((r) => r.id),
    scenes: allRows,
  };
