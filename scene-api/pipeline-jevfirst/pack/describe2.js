#!/usr/bin/env node
// v10.4 (a): --merge applies the text rule A0>C (textrule.js sceneWhy): per flagged scene the strict v10.3 text / title
// (the merge below) when one passed, else the loose one (Jev support >= 0.4, contradicts < 0.3; contradicted,
// placement and direction rules unchanged), re-decided from the Jev answers the why files store. The retry and the
// title pass still trigger on the STRICT result (retryScenes / titleScenes read check-describe.js's strict why).
// whyfinal keeps both merges per scene (strict, loose) next to the shown why.
...
import { mergeWhy, sceneWhy, textRulePolicy, attemptWhy, TEXTRULE_VERSION } from './textrule.js';

const here = path.dirname(fileURLToPath(import.meta.url));
export const DESCRIBE2_VERSION = 'describe2-v10.4';
 ...
 * v10.4: moved unchanged to textrule.js (the A0>C rule merges the strict and the loose attempts with it).
 */
export { mergeWhy } from './textrule.js';
...
/**
 * v10.4 MERGE of a whole film (pure; describe2.js --merge and the tests): per flagged scene of why1, the eligible
 * attempts (attempt 2 only for a retried scene, attempt 3 only for a title-pass scene, each only when checked for
 * the same flag reasons), v10.3's strict merge of the stored whys, the loose merge re-decided from the stored Jev
 * answers, and the shown why (A0>C, textrule.js sceneWhy).
 *   desc         [describe, describe2, describe3] outputs (evidence_ids.w per scene: the plot path's wAllowed)
 *   storedStrict true: the strict whys are the ones check-describe.js stored (production); false: rebuilt from the
 *                rows (round 7 ran with code-built reasons on, so its stored whys are not A0)
 */
export function mergeFilm({ why1, why2 = { scenes: {} }, why3 = { scenes: {} }, desc = [], retried = new Set(), titled = new Set(), pol, storedStrict = true }) {
  const scenes = {};
  ...
  for (const [id, s1] of Object.entries(why1.scenes)) {
    ...
    const att = [s1, ok2 ? s2 : null, ok3 ? s3 : null].map((sc, k) => (sc ? { scene: sc, wList: desc[k]?.scenes?.[id]?.evidence_ids?.w ?? [] } : null));
    // strict = v10.3's merge of the stored whys (check-describe.js buildWhy); loose re-decided from the stored answers
    const r = sceneWhy(att, pol, { strictWhys: storedStrict ? [s1.why, ok2 ? s2.why : null, ok3 ? s3.why : null] : null });
    // the stored strict why and the strict rule re-applied to the same rows must agree (code-built reasons off)
    if (storedStrict) att.forEach((a, k) => { if (!a) return; const w = attemptWhy(a.scene, 'strict', pol, a.wList); if ((w.text ?? null) !== (a.scene.why.text ?? null) || w.title !== a.scene.why.title) strictMismatch.push(`${id} attempt ${k + 1}`); });
    scenes[id] = { reasons: s1.reasons, attempt1: { text: s1.why.text, title_source: s1.why.title_source }, ...(ok2 ? { attempt2: { text: s2.why.text, title: s2.why.title, title_source: s2.why.title_source, checked: s2.checked } } : {}), ...(ok3 ? { attempt3: { title: s3.why.title, title_source: s3.why.title_source, checked: s3.checked } } : {}),
      strict: { text: r.strict.text ?? null, title: r.strict.title, title_source: r.strict.title_source, text_attempt: r.strict.text_attempt, title_attempt: r.strict.title_attempt },
      loose: { text: r.loose.text ?? null, title: r.loose.title, title_source: r.loose.title_source, text_attempt: r.loose.text_attempt, title_attempt: r.loose.title_attempt },
      why: r.why };
  }
  const n = Object.values(scenes);
  return { flagged: n.length, retried: retried.size, title_pass: titled.size, no_text: n.filter((x) => !x.why.text).length, placeholder_title: n.filter((x) => x.why.title_source !== 'sonnet_verified').length, text_strict: n.filter((x) => x.why.text_rule === 'strict').length, text_loose: n.filter((x) => x.why.text_rule === 'loose').length, title_strict: n.filter((x) => x.why.title_rule === 'strict').length, title_loose: n.filter((x) => x.why.title_rule === 'loose').length, strict_no_text: n.filter((x) => !x.strict.text).length, strict_placeholder_title: n.filter((x) => x.strict.title_source !== 'sonnet_verified').length, text_from_attempt2: n.filter((x) => x.why.text_attempt === 2).length, title_from_attempt2: n.filter((x) => x.why.title_attempt === 2).length, title_from_attempt3: n.filter((x) => x.why.title_attempt === 3).length, strict_mismatch: strictMismatch, scenes };
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (argv.includes('--merge')) {
    ...
    // v10.4: the W ids each attempt's describe gave the scene (the plot path's wAllowed)
    const readD = (f) => (fs.existsSync(f) ? JSON.parse(fs.readFileSync(f, 'utf8')) : { scenes: {} });
    const desc = [readD(path.join(OUT, `${slug}.describe.${runId}.json`)), readD(d2f), readD(d3f)];
    const pol = textRulePolicy(JSON.parse(fs.readFileSync(path.join(here, 'policy.json'), 'utf8')));
    const out = { film: why1.film, version: DESCRIBE2_VERSION, textrule: TEXTRULE_VERSION, text_rule: pol.rule, loose_accept: pol.loose.text, run: runId, merged_at: new Date().toISOString(), ...mergeFilm({ why1, why2, why3, desc, retried, titled, pol }) };
    const strictMismatch = out.strict_mismatch;
    fs.writeFileSync(path.join(OUT, `${slug}.whyfinal.${runId}.json`), JSON.stringify(out, null, 2));
    console.log(`${slug}: merged ${out.flagged} flagged (retried ${out.retried}, title pass ${out.title_pass}); text ${out.text_strict} strict + ${out.text_loose} loose, title ${out.title_strict} strict + ${out.title_loose} loose; no text ${out.no_text} (strict alone ${out.strict_no_text}), placeholder title ${out.placeholder_title} (strict alone ${out.strict_placeholder_title})${strictMismatch.length ? `; WARNING stored strict why differs from the rule on ${strictMismatch.join(', ')}` : ''}`);
    process.exit(0);
  }
