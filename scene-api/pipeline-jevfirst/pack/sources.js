#!/usr/bin/env node
// v6 step 0 (unchanged from v5 apart from paths and the held-out gate): fetch the only sources the segmenter may use, and record where they came from.
//
...
import { V8, key, filmMeta, HELD_OUT, heldOutGate } from './env.js';
import { plotSection, splitSentences, numberSentences } from './text.js';

export { HELD_OUT };
const UA = 'TinyViewersResearch/0.6 (scene-safety research experiment; non-commercial)';
...
async function main() {
  ...
  fs.mkdirSync(path.join(V8, 'sources'), { recursive: true });
  const file = path.join(V8, 'sources', `${slug}.json`);
  fs.writeFileSync(file, JSON.stringify(out, null, 2));
  console.log(`${slug}: TMDB ${tmdb.id} "${tmdb.title}" cast ${tmdb.cast.length}/${tmdb.cast_total}; Wikipedia "${wikipedia.title}" rev ${wikipedia.revision_id} (${wikipedia.verified_by}), section "${wikipedia.section}", ${wikipedia.sentence_count} sentences, ${wikipedia.word_count} words -> ${path.relative(V8, file)}`);
}
