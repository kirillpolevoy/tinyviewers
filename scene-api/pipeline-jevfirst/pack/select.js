#!/usr/bin/env node
// v10.4 (b): every FLAGGED scene carries why_tags (textrule.js whyTags): its flag reasons as plain tags (their labels,
// ranked by reasons.js rankReasons, one per distinct label, joined by ' · '), each with the reason ids behind it and
// which model's answer raised it (jev / sonnet). Independent of the parent text: a scene with no verified text still
// says why it is flagged. Flags, tags, severity and spans are v10.3's, unchanged.
// v10.3 (fixes a, b): (a) the MORTAL-DANGER QUESTIONS (mortal.js answers, answers.x.mortal_<q>@<state>): each question
// policy flag.mortal keeps (measured tier A on the 19 seen films, dev/mortal.mjs) adds its reason (deadly_fall /
// bomb_danger) at its threshold; it passes the tier-A gate like the rule-1 question and is cancelled only by
// retold / imagined (the questions' own wording excludes comedy, games, dreams, retellings and songs). (b) the
// CO-OCCURRENCE RULE (policy flag.gating.cooccur): a gated reason named there (afraid_for_safety, tier C) passes the
// gate only when the scene also holds a TIER-A EVENT at act -- an event or film-event tag of a tier-A Jev concept
// (not film-level context), or a kept mortal reason; it then flags with it and never alone.
...
import { MORTAL_REASONS, MORTAL_BY_REASON } from './mortal.js';
import { bridgeSpans } from './bridge.js';
import { whyTags } from './textrule.js';
...
export function severity(answers, cfg, reasons, modifiers, items = [], flagged = false) {
  ...
  for (const band of BANDS) {
    ...
    const content = reasons.reduce((m, t) => {
      const weight = t.id === cfg.parent_tag.id ? cfg.parent_tag.weights[band] : byId.get(t.id)?.weights?.[band] ?? ITEMS[t.id]?.weights?.[band] ?? MORTAL_BY_REASON[t.id]?.weights?.[band] ?? 0;
...
export function passesGate(reason, gating, conceptByV9, items = []) {
  ...
  if (reason.rule === 'rule1_question' || reason.rule === 'mortal_question') return true;
...
/**
 * v10.3 fix (a): the reasons of the mortal-danger questions policy.flag.mortal keeps: for each kept { q, state, t },
 * answers.x['mortal_<q>@<state>'] >= t adds MORTAL_REASONS[q] (one reason per reason id, the highest p). Cancelled
 * by retold / imagined only. [] when disabled or unanswered.
 */
export function mortalReasons(answers, cfg, modifiers) {
  const m = cfg.flag.mortal;
  if (!m?.enabled) return [];
  if (['retold', 'imagined'].some((k) => modifiers[k]?.on)) return [];
  const by = new Map();
  for (const k of m.questions ?? []) {
    const p = Number(answers.x?.[`mortal_${k.q}@${k.state}`]);
    if (!Number.isFinite(p) || p < k.t) continue;
    const meta = MORTAL_REASONS[k.q];
    const prev = by.get(meta.id);
    if (!prev || p > prev.p) by.set(meta.id, { id: meta.id, label: meta.label, group: meta.group, p: r3(p), source: `mortal:${k.q}@${k.state}`, rule: 'mortal_question', by: 'jev' });
  }
  return [...by.values()];
}

/**
 * v10.3 fix (b): does the scene hold a TIER-A EVENT at act? An event / film-event tag at act whose concept is a
 * tier-A Jev concept (policy gating.tier_a_jev_concepts), not film-level context, or a kept mortal reason. Returns the
 * ids found ([] = none).
 */
export function tierAEvents(tags, gating, conceptByV9, items, context, mortal = []) {
  if (!gating) return [];
  const ev = tags.filter((t) => t.level === 'act' && !context.has(t.id) && (t.layer === 'event' || (t.layer === 'film' && t.type !== 'presence')) && gating.tier_a_jev_concepts.includes(conceptOf(t, items, conceptByV9)));
  return [...ev.map((t) => t.id), ...mortal.map((r) => r.id)];
}
...
export function selectScene(answers, cfg, { items = [], context = new Set(), byOf = null, conceptByV9 = null } = {}) {
  ...
  // v10.3 fix (a): the kept mortal-danger questions' reasons (one per reason id)
  const mortal = mortalReasons(answers, cfg, base.modifiers);
  for (const m of mortal) if (!candidates.some((x) => x.id === m.id)) candidates.push(m);
  // ... and shown as tags of the scene too (round 8: Incredibles S006, a bomb on a child's cape, had no such tag)
  for (const m of mortal) if (!base.tags.some((t) => t.id === m.id)) base.tags.push({ id: m.id, group: m.group, label: m.label, layer: 'event', level: 'act', p: m.p, source: m.source });
  ...
  // v10.3 fix (b): the tier-A events of the scene, for the co-occurrence rule
  const tierA = tierAEvents(base.tags, G, conceptByV9, items, context, mortal);
  for (const r of candidates) {
    const g = guardCancels(r, answers, cfg.flag.resolution_guard);
    if (g.length) { guarded.push({ ...r, by_guard: g }); continue; }
    if (!passesGate(r, G, conceptByV9, items)) {
      const co = G?.cooccur?.[r.id] ? tierA.filter((id) => id !== r.id) : [];
      if (co.length) { reasons.push({ ...r, rule: `${r.rule}+cooccur`, cooccur: co }); continue; }
      gated.push({ ...r, concept: conceptOf(r, items, conceptByV9) }); continue;
    }
    reasons.push(r);
  }
...
export function selectRun(run, cfg0, { moments = null, cues = null, reasons = null, why = null, sonnet = null, used = [], childcry = null, resolve = null, split = null, mortal = null } = {}) {
  ...
  const rows = run.scenes.map((row) => {
    ...
    // v10.3: the mortal-danger answers (mortal.js) as answers.x['mortal_<q>@<state>']
    const mv = mortal?.scenes?.[row.id];
    const x = { ...(merged.x ?? {}), ...(mv ? Object.fromEntries(Object.entries(mv).map(([k, p]) => [`mortal_${k}`, p])) : {}), ...(cc ? { crying_child: cc.p_child } : {}), ...(rv?.guard ? Object.fromEntries(Object.keys(GUARD_QS).filter((k) => rv.guard[k] != null).map((k) => [`guard_${k}`, rv.guard[k]])) : {}), ...(rv?.r1 != null ? { r1_threat: rv.r1 } : {}) };
  ...
  const scenes = rows.map((row, i) => {
    ...
    return { ...baseRow, ...r, ...(row.answers?.sonnet_missing ? { sonnet_missing: true } : {}), skip: skipFor(baseRow, r.flagged, moments?.scenes), ...(r.flagged ? { why: wy, why_tags: whyTags(r.flag_reasons, items) } : {}) };
  });
  // v10.3 fix (c): the span bridge across scene boundaries (bridge.js), once the moment spans exist
  if (cfg.spans_bridge?.enabled && moments) {
    const br = bridgeSpans(scenes, cfg.spans_bridge, cues);
    for (const s of scenes.filter((x) => x.flagged)) s.skip = { ...s.skip, spans: br[s.id].spans, ms: br[s.id].ms, ...(br[s.id].added.length ? { bridge: br[s.id].added } : {}) };
  }
  ...
  return {
    ...
    summary: {
      ...
      // v10.3: scenes with a mortal-danger reason / a co-occurrence reason (afraid_for_safety with a tier-A event)
      mortal_reasons: flagged.filter((s) => s.flag_reasons.some((r) => r.rule === 'mortal_question')).map((s) => `${s.id}:${s.flag_reasons.filter((r) => r.rule === 'mortal_question').map((r) => r.id).join('+')}`),
      cooccur_reasons: flagged.filter((s) => s.flag_reasons.some((r) => r.cooccur)).map((s) => s.id),
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  // v10.3 fix (d): the merged parent text (describe2.js --merge) when it exists, else check-describe's first attempt
  const finalFile = path.join(OUT, `${slug}.whyfinal.${runId}.json`);
  const whyFile = fs.existsSync(finalFile) ? finalFile : path.join(OUT, `${slug}.why.${runId}.json`);
  ...
  // v10.3: the mortal-danger answers (mortal.js); required when policy.flag.mortal is enabled
  const mvFile = path.join(OUT, `${slug}.mortal.${runId}.json`);
  const mortal = fs.existsSync(mvFile) ? JSON.parse(fs.readFileSync(mvFile, 'utf8')) : null;
  if (!mortal && policy.flag.mortal?.enabled) { console.error(`${slug}: ${path.relative(here, mvFile)} does not exist (run mortal.js)`); process.exit(2); }
  const out = selectRun(run, policy, { moments, cues, why, sonnet, used: split.sonnet_used, childcry, resolve, split, mortal });
  out.mortal_file = mortal ? path.relative(here, mvFile) : null;
  ...
  console.log(file);
  console.log(`  flagged ${s.flagged}/${s.scenes} scenes (reasons by sonnet ${s.flag_reasons_by.sonnet}, jev ${s.flag_reasons_by.jev}; ${s.flag_reasons_by.scenes_flagged_only_by_sonnet} scenes only by sonnet); flagged scenes ${min(s.flagged_scene_ms)} min, skip ${min(s.skip_ms)} min (${out.moments_file ? 'moments' : 'whole scenes'}); why: described ${s.why.described}, +reason ${s.why.described_plus_reason}, described only ${s.why.described_only}, plain ${s.why.plain_reason}, no verified text ${s.why.no_verified_text}, missing ${s.why.missing.length ? s.why.missing.join(',') : 0}; text strict ${s.why.text_strict} / loose ${s.why.text_loose}, title strict ${s.why.title_strict} / loose ${s.why.title_loose}; why tags missing ${s.why.why_tags_missing.length ? s.why.why_tags_missing.join(',') : 0}`);
  console.log(`  gate: ${s.gated_only_scenes.length} scenes held only tier-B/C reasons (tags only)${s.gated_only_scenes.length ? ` ${s.gated_only_scenes.join(',')}` : ''}; guard cancelled all reasons in ${s.guard_only_scenes.length}${s.guard_only_scenes.length ? ` ${s.guard_only_scenes.join(',')}` : ''}; rule-1 question reasons in ${s.rule1_reasons.length}; mortal reasons ${s.mortal_reasons.join(' ') || '-'}; co-occurrence reasons ${s.cooccur_reasons.join(',') || '-'}`);
