...
- `TINY_VIEWERS_DEV_POSTERS=1 TMDB_API_KEY=…` — look posters up for real instead of drawing the
  placeholder.
...
| `TMDB_API_KEY` | the scene-api loader only | no — without it `poster_url` is null and the UI draws its placeholder |
...
| --- | --- |
| `app/` | Routes: `/`, `/library`, `/search`, `/film/[slug]`, `/watch`, `/watch/[slug]`, plus `not-found`, `error` and `global-error` |
...
| `lib/search.ts` | Name matching: case, punctuation and accents — pure, and tested |
