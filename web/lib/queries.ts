...
export const getFilmScenes = cache(async (slug: string): Promise<Scene[]> => {
  ...
  const { rows } = await db.query<SceneRow>(
    ...
    `select s.id, s.start_ms, s.end_ms, s.title, s.description,
            ...
            -- Read through to_jsonb so a database whose scenes have no why_tags column yet (a guide
            -- built before v10.4) answers null instead of failing the page.
            to_jsonb(s) -> 'why_tags' as why_tags
       from scenes s
