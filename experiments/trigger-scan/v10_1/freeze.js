...
const v10Freeze = path.join(V8, '..', 'v10', 'out10', 'freeze.json');
const parent = fs.existsSync(v10Freeze) ? { file: path.relative(V8, v10Freeze), sha256: crypto.createHash('sha256').update(fs.readFileSync(v10Freeze)).digest('hex'), frozen_at: JSON.parse(fs.readFileSync(v10Freeze, 'utf8')).frozen_at } : null;
fs.writeFileSync(file, JSON.stringify({ frozen_at: new Date().toISOString(), version: 'v10.1', parent_freeze: parent, jev_set: JEV_SET_VERSION, jev_set_fix_mode: FIX_MODE, allow_proxy: allowProxy, decisions_sha256_12: dsha, dev_films: DEV_FILMS, note: 'sha256 of every file that decides what a model is asked, how answers become sentences, flags, skip spans, a split verdict or a why, or which films may run. Held-out films are refused unless all still match.', files }, null, 2));
