...
// v7 span behaviour (v8's wordless.extend is tested in v8-spans.test.js)
const cfg0 = loadPolicy();
const cfg = { ...cfg0, wordless: { ...cfg0.wordless, extend: null } };
...
test('planScene (v10): one request per distinct state; the lines state never sees summary or cast; empty summary drops the summary questions', () => {
  const items = filmItems(seg);
  const p1 = planScene({ seg, scene: seg.scenes[0], cues: cues.slice(0, 3), items });
  const keysOf = (r) => Object.values(r.back).map((b) => b.key);
  // v9-kept questions: mention + film presence on the lines state, modifiers / Scores / kind / film summary presence on V9C
  const lines = p1.reqs.find((r) => keysOf(r).some((k) => k.startsWith('m.')));
  assert.deepEqual(Object.keys(lines.body.state), ['film', 'scene']);
  assert.deepEqual(Object.keys(lines.body.state.scene), ['lines']);
  assert.ok(!JSON.stringify(lines.body.state).includes('Vera talks'));
  assert.ok(keysOf(lines).includes('fpl.C01_present'));
  const ctx = p1.reqs.find((r) => keysOf(r).includes('kind'));
  assert.ok(keysOf(ctx).includes('mod.retold') && keysOf(ctx).includes('s.danger') && keysOf(ctx).includes('fps.C01_present'));
  // every jev-set question is asked exactly once per scene (templates once per item), none twice
  const jev = p1.reqs.flatMap((r) => Object.values(r.back).filter((b) => b.kind === 'jev').map((b) => b.key));
  assert.equal(new Set(jev).size, jev.length);
  assert.ok(jev.length > 50);
  // request question keys are opaque (q1..qN): the question ids never reach the model
  for (const r of p1.reqs) assert.ok(Object.keys(r.body.questions).every((k) => /^q\d+$/.test(k)));
  const p2 = planScene({ seg, scene: seg.scenes[1], cues: cues.slice(3), items });
  assert.equal(p2.summaryEmpty, true);
  const k2 = p2.reqs.flatMap(keysOf);
  assert.ok(!k2.some((k) => /^fps\.|^V:ps\.|@S$|@SC$/.test(k)), 'no summary questions without a verified summary');
});
...
test('unpackAnswers (v10): every asked question must come back; jev-set answers land in answers.q by key', () => {
  const req = { body: { questions: { q1: { type: 'noul' }, q2: { type: 'score' }, q3: { type: 'choice' }, q4: { type: 'noul' } } }, back: { q1: { key: 'C:chased.c@S', kind: 'jev' }, q2: { key: 's.laughs', kind: 'v9' }, q3: { key: 'kind', kind: 'v9' }, q4: { key: 'm.fire', kind: 'v9' } } };
  const a = unpackAnswers(req, { q1: { noul: 0.2 }, q2: { score: 1.2, confidence: 0.5, probabilities: { 0: 0.1, 1: 0.6, 2: 0.3, 3: 0 } }, q3: { choice: 'none', confidence: 1, probabilities: { none: 1 } }, q4: { noul: 0.9 } });
  assert.equal(a.q['C:chased.c@S'], 0.2);
  assert.equal(a.s.laughs.top, 3);
  assert.equal(a.kind.choice, 'none');
  assert.equal(a.m.fire, 0.9);
  assert.throws(() => unpackAnswers(req, { q1: { noul: 0.2 } }), /missing answer/);
});
