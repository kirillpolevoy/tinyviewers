...
test('rule 2: Sonnet crying at act flags only with a child involved (Jev child answer or a cast child present)', () => {
  assert.ok(POLICY.flag.strong_events.includes('crying'));
  assert.deepEqual(POLICY.flag.requires.crying, { child: { cast_presence: true, jev_min: 0.5 } });
  ...
  // (a) a verified cast child's film presence tag at act
  const items = [{ id: 'C01_present', entity: 'C01', type: 'presence', why: 'child', name: 'Kid', label: 'Kid', group: 'people' }];
  const cast = selectScene(base({ e: { crying: 0.8 }, fpl: { C01_present: 0.9 } }), CFG, { items });
  assert.equal(cast.flagged, true);
  assert.equal(cast.flag_reasons[0].child, 'cast_child');
  assert.equal(childInvolved({ cast_presence: true, jev_min: 0.5 }, { x: {} }, []), null);
});
...
const CC = path.join(V101, 'out101', 'dev', 'good-dinosaur.childcry.r1.json');
