...
test('held-out gate (v10.1): the 14 seen films are dev (good-dinosaur since round 6); every other slug (frozen, zootopia, ...) needs --final-held-out-run and an intact out101/freeze.json', () => {
  assert.deepEqual([...DEV_FILMS].sort(), ['book-of-life', 'coco', 'frankenweenie', 'good-dinosaur', 'how-to-train-your-dragon', 'iron-giant', 'lion-king', 'moana', 'monsters-inc', 'nemo', 'princess-and-the-frog', 'tangled', 'up', 'wild-robot']);
  assert.equal(HELD_OUT.has('frozen'), true);
  assert.equal(HELD_OUT.has('zootopia'), true);
