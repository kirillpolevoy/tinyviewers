...
const N = 9;
const FILMS = ['nemo', 'monsters-inc', 'lion-king', 'frankenweenie', 'wild-robot', 'iron-giant', 'up', 'tangled', 'coco', 'how-to-train-your-dragon', 'book-of-life', 'princess-and-the-frog', 'moana', 'good-dinosaur'];
const grams = {};
...
add(path.join(V101, 'out101'));
add(path.join(V101, 'dev', 'out'));
add(path.join(V101, 'eval', 'out'));
add(path.join(V101, 'labels', 'codex'), (x) => x.startsWith('v101') && x.endsWith('.answer.json'));
...
fs.mkdirSync(path.join(V101, 'verify', 'out'), { recursive: true });
fs.writeFileSync(path.join(V101, 'verify', 'out', 'quote.json'), JSON.stringify(res, null, 2));
