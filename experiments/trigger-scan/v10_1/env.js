...
export const DEV_FILMS = ['nemo', 'monsters-inc', 'lion-king', 'frankenweenie', 'wild-robot', 'iron-giant', 'up', 'tangled', 'coco', 'how-to-train-your-dragon', 'book-of-life', 'princess-and-the-frog', 'moana', 'good-dinosaur'];
...
export function outDir() {
  // v10.1: $V101_OUT (relative to v10_1/) or v10_1/out101. v10's outputs (v10/out, v10/out10) are read-only inputs.
  const d = process.env.V101_OUT ? path.resolve(V8, process.env.V101_OUT) : path.join(V8, 'out101');
...
export function heldOutGate(slug, argv = process.argv) {
  ...
  if (!st.ok) {
    console.error(`${slug} is held out and ${st.missing ? 'out101/freeze.json does not exist (run node freeze.js first)' : `these frozen files changed since ${st.frozen_at}: ${st.changed.join(', ')}`}. Refusing.`);
    process.exit(2);
  }
}
