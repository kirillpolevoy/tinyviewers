 ...
 * cries a child?", answers.x.crying_child) >= req.jev_min.
 */
...
export function selectScene(answers, cfg, { items = [], context = new Set() } = {}) {
  ...
  for (const t of base.tags) {
    ...
    const r = { id: t.id, label: t.label, group: t.group, p: t.p, source: t.source, rule, ...(t.film_specific ? { film_specific: true } : {}), ...(childReq ? { child: childInvolved(childReq, answers, base.tags) } : {}) };
...
export function selectRun(run, cfg0, { moments = null, cues = null, reasons = null, why = null, sonnet = null, used = [], childcry = null } = {}) {
  ...
  const rows = run.scenes.map((row) => {
    ...
    return { ...row, answers: cc ? { ...merged, x: { ...(merged.x ?? {}), crying_child: cc.p_child } } : merged };
  });
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!childcry) console.error(`${slug}: WARNING no ${path.relative(here, ccFile)} (run childcry.js): crying flags only through a cast child's presence`);
  const out = selectRun(run, loadPolicy(polFile), { moments, cues, why, sonnet, used: split.sonnet_used, childcry });
