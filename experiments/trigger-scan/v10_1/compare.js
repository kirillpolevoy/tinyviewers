#!/usr/bin/env node
// v8 scoring (v7's compare.js, relabelled; + the three PARENT CHECKS). Pure code: no model calls, no
// network, no database.
//
//   node compare.js [--films nemo,...] [--runs r1,r2] [--refs out/refs-human] [--before ../v7/out]
//                   (V8_OUT selects the output directory, default out/)
//
// BEFORE = v7 AS RUN (../v7/out/<slug>.tags.r1.json and its segments / reasons measured by reasons.js
// --measure, read-only): every metric below is computed for v8 and for v7 as run with the same
// functions, and a code-computed verdict v8 vs v7 (refscore.js compareSystems, jitter).
//
// PRIMARY: the HUMAN-WRITTEN reference key (out/refs-human/<slug>.key.json = refs/<slug>.key.json
// without the model-written 'codex-rules' items; eval/human-keys.mjs), as v7's evaluation used:
//   recall of should_flag items on the strict span AND the gap window (conservative = the lower),
//   should_flag-only precision proxy, tag-only (comic) items skipped (target 0), jitter, DTDD.
// PARENT CHECKS (parent-checks.js): (i) wordless-peak coverage, (ii) wordless-scene detection,
// (iii) why for every flagged scene (reasons.js).
// SECONDARY: the live DB baseline (scene-api/load.js, read-only). The reference key is never read by
// anything but this file, refscore.js, parent-checks.js and dev/ tuning scripts.
//
// Writes <out>/summary.json and <out>/explain.json.
...
import { V8, TS, outDir, HELD_OUT, DEV_FILMS, freezeStatus } from './env.js';
...
const BEFORE = path.resolve(V8, opt('before', '../v7/out'));
const [RA, RB] = opt('runs', 'r1,r2').split(',');
const REFS = path.resolve(V8, opt('refs', 'out/refs-human'));
const OUT = outDir();
...
for (const slug of FILMS) {
  ...
  if (missing.length) { problems.push(`${slug}: missing ${missing.join(', ')} in ${path.relative(V8, OUT)}; film skipped`); continue; }
  const held = HELD_OUT.has(slug);
  const seg = readJson(path.join(OUT, `${slug}.segments.json`));
  const A = sysFor(slug, RA);
  const B = RB && exists(path.join(OUT, `${slug}.tags.${RB}.json`)) && exists(path.join(OUT, `${slug}.jev.${RB}.json`)) ? sysFor(slug, RB) : null;
  ...
  const claimCheck = { v8: verifiedShare(seg), ...(!held && exists(v5seg) ? { v5_round1: verifiedShare(readJson(v5seg)) } : {}) };
  ...
  const liveDb = {
    ...
    v8_flags_outside: outsideBase.map((v) => v.id),
  ...
  let reference = null;
  if (!exists(keyFile)) problems.push(`${slug}: no reference key ${path.relative(V8, keyFile)}`);
  else {
    const key = readJson(keyFile);
    const dbSpans = base.scenes.map((b) => [b.start_ms, b.end_ms]);
    const aScore = scoreKey(key, skipSpans, { liveDb: base.scenes.length ? dbSpans : null });
    const aGap = scoreKey(key, skipSpans, { liveDb: base.scenes.length ? dbSpans : null, window: 'gap' });
    const aByScenes = scoreKey(key, flagged.map((s) => [s.start_ms, s.end_ms]));
    const aByScenesGap = scoreKey(key, flagged.map((s) => [s.start_ms, s.end_ms]), { window: 'gap' });
    ...
    // v7 as run (the 'before'), dev films only
    const beforeFile = path.join(BEFORE, `${slug}.tags.r1.json`);
    let before = null;
    if (!held && exists(beforeFile)) {
      const r2 = readJson(beforeFile);
      ...
      const r2Flagged = r2.scenes.filter((x) => x.flagged);
      const beforeReasons = path.join(OUT, 'before', `${slug}.reasons.r1.json`);
      before = {
        file: path.relative(V8, beforeFile), scenes: r2.scenes.length, flagged: r2Flagged.length, strict: r2s, gap: r2g, sens: sensR2, verdict: compareSystems({ aStrict: aScore, aGap: aGap, bStrict: r2s, bGap: r2g, sens: sensR2 }),
        dtdd: dtddAgreement(key, r2.scenes.flatMap((x) => (x.tags ?? []).filter((t) => t.level === 'act').map((t) => ({ id: t.id, group: t.group })))),
        parent: {
          i_wordless_peaks: checkWordlessPeaks(key, r2Flagged, r2Skip),
          ii_wordless_scenes: checkWordlessScenes(key, wordlessScenes(seg, cues, POLICY), r2Skip),
          iii_why: exists(beforeReasons) ? checkWhy(readJson(beforeReasons), r2Flagged.map((x) => x.id), { generatedCounts: false }) : null,
        },
      };
    }
    const reasonsFile = path.join(OUT, `${slug}.reasons.${RA}.json`);
    const parentA = {
      i_wordless_peaks: checkWordlessPeaks(key, flagged, skipSpans),
      ii_wordless_scenes: checkWordlessScenes(key, wordlessScenes(seg, cues, POLICY), skipSpans),
      iii_why: exists(reasonsFile) ? checkWhy(readJson(reasonsFile), flagged.map((x) => x.id)) : null,
    };
    const verdict = base.scenes.length ? compareSystems({ aStrict: aScore, aGap: aGap, bStrict: dbScore, bGap: dbGap, sens }) : null;
    // the key barely covers policy rules 1 (explicit villain threat) and 2 (child terrified): precision
    // with and without the v8 scenes flagged ONLY for a threat, a plan to harm or a frightened child
    ...
    const rule12 = {
      ...
      v8_scenes_flagged_only_by_rule_1_or_2: onlyRule12,
      ...
      v8_without_those_scenes: (() => { const a = scoreKey(key, noRule12); const g = scoreKey(key, noRule12, { window: 'gap' }); return { skip_minutes: a.skip_minutes, recall_strict: `${a.recall_items.found}/${a.recall_items.of}`, recall_gap: `${g.recall_items.found}/${g.recall_items.of}`, precision_strict: a.precision_proxy_ref_only, precision_gap: g.precision_proxy_ref_only }; })(),
    ...
    const dtddA = dtddAgreement(key, actTags);
    ...
    reference = {
      file: path.relative(V8, keyFile), sources_used: (key.sources_used ?? []).map((s) => s.source), counts: key.counts ?? null,
      items: aScore.items_total, mapped: aScore.mapped, unmappable: aScore.unmappable.length, unmappable_items: aScore.unmappable,
      v8_skip: brief(aScore), v8_flagged_scenes: brief(aByScenes), live_db: brief(dbScore),
      gap_window: { v8_skip: brief(aGap), v8_flagged_scenes: brief(aByScenesGap), live_db: brief(dbGap) },
      sensitivity_v8_vs_live_db: sens,
      verdict_v8_vs_live_db: verdict,
      policy_rules_1_2: rule12,
      before_v7_as_run: before && { file: before.file, scenes: before.scenes, flagged: before.flagged, skip: brief(before.strict), gap_window: brief(before.gap), sensitivity_v8_vs_v7: before.sens, verdict_v8_vs_v7: before.verdict, dtdd: dtddBrief(before.dtdd) },
      parent_checks: { v8: parentA, v7_as_run: before?.parent ?? null },
      dtdd: { v8: dtddBrief(dtddA), live_db: dtddBrief(dtddDb) },
    };
    // explanations: every missed should_flag item, with the scenes it overlaps
    for (const m of aScore.missed) {
      ...
      explain.push({
        ...
        share_in_v8_skip: m.share_in_skip, share_in_flagged_scenes: r3(aByScenes.rows.find((r) => r.id === m.id)?.share_in_skip ?? 0), share_in_live_db: r3(dbScore?.rows.find((r) => r.id === m.id)?.share_in_skip ?? null),
        v8_scenes: touching.map((s) => ({
    ...
    // v8 flags whose skip matches neither the key nor the live DB
...
const summary = {
  ...
  definitions: {
    ...
    dtdd: 'DoesTheDogDie topics with a yes/no answer; v8 yes when any scene has an act-level tag the topic maps to (refscore.js dtddTargets)',
    live_db_coverage: 'live-DB scene >= 50% of its time inside v8 skip spans / flagged scenes; outside = flagged v8 scene whose dialogue bounds overlap no live-DB scene',
  },
  freeze: freeze.missing ? 'no out/freeze.json' : { frozen_at: freeze.frozen_at, intact: freeze.ok, changed: freeze.changed },
  films: perFilm,
  verdict_v8_vs_live_db: Object.fromEntries(perFilm.filter((f) => f.reference?.verdict_v8_vs_live_db).map((f) => { const v = f.reference.verdict_v8_vs_live_db; const w = (x) => (x === 'A' ? 'v8' : x === 'B' ? 'live DB' : 'tie'); return [f.slug, { held_out: f.held_out, recall: w(v.recall), precision: w(v.precision), overall: v.overall === 'A' ? 'v8' : v.overall === 'B' ? 'live DB' : v.overall, conservative_recall: `${v.conservative_recall.a} vs ${v.conservative_recall.b} of ${v.conservative_recall.of}`, precision_strict: `${v.precision_strict.a} vs ${v.precision_strict.b}`, precision_gap: `${v.precision_gap.a} vs ${v.precision_gap.b}`, skip_minutes: `${v.skip_minutes.a} vs ${v.skip_minutes.b}` }]; })),
  verdict_v8_vs_v7: Object.fromEntries(perFilm.filter((f) => f.reference?.before_v7_as_run).map((f) => { const r = f.reference.before_v7_as_run; const v = r.verdict_v8_vs_v7; const w = (x) => (x === 'A' ? 'v8' : x === 'B' ? 'v7 as run' : 'tie'); return [f.slug, { recall: w(v.recall), precision: w(v.precision), overall: v.overall === 'A' ? 'v8' : v.overall === 'B' ? 'v7 as run' : v.overall, conservative_recall: `${v.conservative_recall.a} vs ${v.conservative_recall.b} of ${v.conservative_recall.of}`, precision_strict: `${v.precision_strict.a} vs ${v.precision_strict.b}`, precision_gap: `${v.precision_gap.a} vs ${v.precision_gap.b}`, skip_minutes: `${v.skip_minutes.a} vs ${v.skip_minutes.b}`, tag_only_flagged: `${f.reference.v8_skip.tag_only.flagged} vs ${r.skip.tag_only.flagged} of ${r.skip.tag_only.items}`, tag_only_minutes: `${f.reference.v8_skip.skip_minutes_over_non_flag_items.tag_only} vs ${r.skip.skip_minutes_over_non_flag_items.tag_only}`, jitter_recall_share_v8_ge_v7: r.sensitivity_v8_vs_v7.recall_diff_a_minus_b.share_a_ge_b }]; })),
  problems,
};
...
for (const f of perFilm) {
  ...
  console.log(`claim check: sentences ${cc.v8.sentences_verified} verified, ${cc.v8.scenes_with_verified_summary} scenes / ${cc.v8.minutes_with_verified_summary} min with a verified summary${cc.v5_round1 ? ` (v5: ${cc.v5_round1.sentences_verified}, ${cc.v5_round1.scenes_with_verified_summary}, ${cc.v5_round1.minutes_with_verified_summary} min)` : ''}`);
  ...
  if (f.reference) {
    ...
    line('v8 skip', r.v8_skip); line('v8 scenes', r.v8_flagged_scenes); line('live DB', r.live_db);
    line('v8 skip gap', r.gap_window.v8_skip); line('live DB gap', r.gap_window.live_db);
    if (r.before_v7_as_run) { line('v7 as run', r.before_v7_as_run.skip); line('v7 as run gap', r.before_v7_as_run.gap_window); const v = r.before_v7_as_run.verdict_v8_vs_v7; console.log(`  VERDICT v8 vs v7 as run: recall ${v.recall}, precision ${v.precision}, overall ${v.overall} (A = v8); conservative recall ${v.conservative_recall.a} vs ${v.conservative_recall.b}; jitter v8>=v7 ${r.before_v7_as_run.sensitivity_v8_vs_v7.recall_diff_a_minus_b.share_a_ge_b}`); }
    if (r.sensitivity_v8_vs_live_db) {
      const sx = r.sensitivity_v8_vs_live_db;
      console.log(`  shifts (v8/DB recall): ${Object.entries(sx.shifts).map(([k, x]) => `${k} ${x.a_recall}/${x.b_recall}`).join(', ')}; jitter recall diff p05..p95 ${sx.recall_diff_a_minus_b.p05}..${sx.recall_diff_a_minus_b.p95} (v8>DB ${sx.recall_diff_a_minus_b.share_a_gt_b}, v8<DB ${sx.recall_diff_a_minus_b.share_a_lt_b}); precision diff ${sx.precision_diff_a_minus_b.p05}..${sx.precision_diff_a_minus_b.p95} (v8>DB ${sx.precision_diff_a_minus_b.share_a_gt_b})`);
      const v = r.verdict_v8_vs_live_db;
      console.log(`  VERDICT recall ${v.recall}, precision ${v.precision}, overall ${v.overall} (A = v8, B = live DB); conservative recall ${v.conservative_recall.a} vs ${v.conservative_recall.b}`);
    }
    const pc = r.parent_checks;
    const pcl = (n, x) => x && console.log(`  parent ${n.padEnd(10)} (i) wordless peaks ${x.i_wordless_peaks.covered}/${x.i_wordless_peaks.n} >= 80% in skip${x.i_wordless_peaks.missed.length ? ` [miss ${x.i_wordless_peaks.missed.join(' ')}]` : ''}; (ii) wordless-scene items found ${x.ii_wordless_scenes.found}/${x.ii_wordless_scenes.n}${x.ii_wordless_scenes.missed.length ? ` [miss ${x.ii_wordless_scenes.missed.join(' ')}]` : ''}; (iii) why stated ${x.iii_why?.stated ?? '-'}, generated ${x.iii_why?.generated ?? '-'}, unrelated only ${x.iii_why?.unrelated_only ?? '-'}, nothing ${x.iii_why?.nothing ?? '-'} of ${x.iii_why?.flagged ?? '-'}`);
    pcl('v8', pc.v8); pcl('v7 as run', pc.v7_as_run);
    const q = r.policy_rules_1_2;
    console.log(`  rules 1/2: key marks villain ${q.key_items_marked.villain_threat}, child ${q.key_items_marked.child_terrified} of ${q.key_items_marked.of_should_flag}; v8 scenes only by rule 1/2: ${q.v8_scenes_flagged_only_by_rule_1_or_2.join(',') || '-'} (${q.minutes_only_by_rule_1_or_2} min); without them precision ${q.v8_without_those_scenes.precision_strict}/${q.v8_without_those_scenes.precision_gap} (strict/gap), recall ${q.v8_without_those_scenes.recall_strict}`);
    if (r.dtdd.v8) console.log(`  DTDD (${r.dtdd.v8.scored} of ${r.dtdd.v8.topics} topics map to specific tags) v8 agree ${r.dtdd.v8.agree}/${r.dtdd.v8.scored} (tp ${r.dtdd.v8.tp} tn ${r.dtdd.v8.tn} fp ${r.dtdd.v8.fp} fn ${r.dtdd.v8.fn})${r.dtdd.live_db ? `; live DB ${r.dtdd.live_db.agree}/${r.dtdd.live_db.scored} (tp ${r.dtdd.live_db.tp} tn ${r.dtdd.live_db.tn} fp ${r.dtdd.live_db.fp} fn ${r.dtdd.live_db.fn})` : ''}`);
  }
  const d = f.live_db;
  console.log(`live DB: covered by skip ${d.covered_by_skip}, by flagged ${d.covered_by_flagged_scenes}; v8 flags outside ${d.v8_flags_outside.length} (${d.v8_flags_outside.join(',') || '-'}); tag F1 item ${d.tag_agreement.item_v3.f1} group ${d.tag_agreement.group_13.f1}`);
...
console.log(`\nexplain: ${explain.length} items -> ${path.relative(V8, path.join(OUT, 'explain.json'))}; freeze ${JSON.stringify(summary.freeze)}`);
