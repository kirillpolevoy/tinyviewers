...
const BASE = process.env.R5_BASE ? path.resolve(process.env.R5_BASE) : path.join(V8, '..', 'v10', 'round6', 'live'); // R5_BASE only to validate this scorer on round-4 films
const R5OUT = path.join(V8, 'round7', 'out');
const TAGS_DIR = process.env.R7_TAGS_DIR ? path.resolve(process.env.R7_TAGS_DIR) : null;
...
fs.writeFileSync(path.join(R5OUT, process.env.R7_NAME ?? 'headtohead.json'), JSON.stringify(result, null, 2));
