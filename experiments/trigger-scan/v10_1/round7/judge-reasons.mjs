...
function build(slug) {
  ...
  const r = rng(seedOf(`r7-reasons:${slug}`));
...
const results = await Promise.all(FILMS.map(async (slug) => {
  ...
  const r = await runCodex(`r7-reasons-${slug}`, prompt, schema);
...
fs.writeFileSync(path.join(here, 'out', 'reasons-truth.json'), JSON.stringify({ generated_at: new Date().toISOString(), judge: 'codex exec gpt-6-astra high read-only ephemeral (round7/judge-reasons.mjs)', per_film: results }, null, 2));
