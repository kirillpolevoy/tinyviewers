// Question set v5 — two layers of Jev questions per SCENE, and the states they are asked against.
//
...
export const PL_YES = 'The lines show it is there now: a character sees it, speaks to it, touches it, or reacts to it, or a sound caption describes it. Friendly, funny, imagined and dangerous ones all count.';
...
export const PRESENCE = [
  ...
  P('reanimated_dead', 'reanimated_dead', 'creatures_figures', 'Undead', 'a dead person or animal brought back to life', 3, 2, { moment: 'a dead person or animal is brought back to life' }),
  ...
  P('dangerous_machine', 'dangerous_machine', 'objects_hazards', 'Dangerous machinery', 'dangerous machinery or live electricity', 2, 2, {
    ...
    // v4 finding 5: SDH sound captions alone fired this item
    plNoExtra: 'A sound caption of a machine (a hum, a whir, a beep, a buzz, a clank) is a no unless the lines also show the machine catching, hitting, pulling in, or shocking a character.',
...
export const EVENTS = [
  ...
  // death & loss
  E('dies', 'dies', 'death', 'Someone dies', 'In `scene`, does a character die or get killed?', `${RETOLD} Pretending to be dead is a no.`, 3, 3, { cancel: PHYSICAL, moment: 'a character dies or is killed' }),
  E('loved_one_dies', 'dies', 'death', 'A loved one dies', "In `scene`, does a character's family member or close friend die?", RETOLD, 3, 3, { cancel: PHYSICAL, v2: 'loved_one_dies', moment: "a character's family member or close friend dies" }),
  E('pet_dies', 'dies', 'death', 'A pet dies', 'In `scene`, does a pet die?', RETOLD, 3, 3, { cancel: PHYSICAL, v2: 'pet_animal_dies', moment: 'a pet dies' }),
  ...
  // separation & family
  E('child_taken', 'child_taken', 'separation', 'Child taken', 'In `scene`, is a child taken away from their parent or carer?', 'A parent telling others that their child was taken earlier is a no. A school drop-off is a no.', 3, 3, { cancel: PHYSICAL, moment: 'a child is taken away from their parent or carer' }),
  E('child_separated', 'child_lost', 'separation', 'Child lost', 'In `scene`, is a child separated from their parent or carer and unable to find them?', RETOLD, 3, 3, { cancel: PHYSICAL, moment: 'a child is separated from their parent or carer' }),
  ...
  E('threatens_harm', 'threatens_harm', 'hostility', 'Threats', 'In `scene`, does a character threaten to hurt another character?', null, 2, 2, { cancel: { comic: true }, moment: 'a character threatens to hurt another character' }),
  ...
  // animals
  E('animal_cruelty', 'animal_cruelty', 'animals', 'Animal mistreated', 'In `scene`, does a character deliberately hurt or mistreat an animal?', RETOLD, 3, 3, { cancel: PHYSICAL, moment: 'a character deliberately hurts an animal' }),
...
export const ACCEPT_CONFIDENCE = 0.8; // citation_check cookbook: AUTO_ACCEPT

/**
 * Was this claim verified by the claim check? Accepts the shapes the check may write:
 *   { verdict: 'supports', confidence } (accept at >= ACCEPT_CONFIDENCE), { accepted: true },
 *   { status: 'verified' }. Anything else (says_nothing, contradicts, low confidence, missing) is not.
 */
 ...
 * (check.disposition = {verdict, confidence}) or one check for the whole entity (check = {verdict,...}).
 */
...
export function castView(c) {
  const disposition = verifiedField(c, 'disposition');
  const row = {
    name: c.name,
    ...(c.aliases?.length ? { also_called: c.aliases } : {}),
    kind: verifiedField(c, 'kind'),
    is_child: verifiedField(c, 'is_child'),
    looks_frightening: verifiedField(c, 'looks_frightening'),
    disposition,
  };
  if (disposition !== 'unknown' && c.disposition_note) row.note = c.disposition_note;
  return row;
}

export const dangerView = (d) => ({ name: d.name, kind: d.kind, ...(d.note ? { note: d.note } : {}) });

const nameList = (c) => {
  const extra = (c.aliases ?? []).filter((a) => a && a.toLowerCase() !== c.name.toLowerCase());
 ...
 *   villain / threat / changes disposition, or looks_frightening true -> presence (2) + threatens (1)
 *   is_child true                                                     -> presence (2) + in danger (1)
 *   each verified danger                                              -> used on / endangers (1)
 * Order: villains, threats, children, dangers, 'changes', frightening-looking others.
 */
export function filmItems(seg, { cap = FILM_CAP } = {}) {
  const buckets = { villain: [], threat: [], child: [], danger: [], changes: [], frightening: [] };
  for (const [ci, c0] of (seg.cast ?? []).entries()) {
    ...
    if (['villain', 'threat', 'changes'].includes(disp) || scary) {
      const bucket = disp === 'villain' ? 'villain' : disp === 'threat' ? 'threat' : disp === 'changes' ? 'changes' : 'frightening';
      buckets[bucket].push([
        { ...presence, id: `${c.id}_present`, why: bucket },
        { entity: c.id, name: c.name, id: `${c.id}_threatens`, type: 'threatens', why: bucket, group: 'peril', v3: 'attacked', label: `${c.name} threatens someone`, who, weights: { '5-7': 3, '8-10': 3 }, cancel: PHYSICAL, moment: `${c.name} threatens, chases, or attacks someone` },
      ]);
    }
    if (child) {
      buckets.child.push([
        { ...presence, id: `${c.id}_present`, why: 'child' },
        { entity: c.id, name: c.name, id: `${c.id}_in_danger`, type: 'child_in_danger', why: 'child', group: 'peril', v3: 'family_in_danger', label: `${c.name} in danger`, who, weights: { '5-7': 3, '8-10': 2 }, cancel: PHYSICAL, moment: `${c.name} is in danger` },
      ]);
    }
  }
  for (const [di, d0] of (seg.dangers ?? []).entries()) {
    ...
    buckets.danger.push([{ entity: d.id, name: d.name, id: `${d.id}_endangers`, type: 'danger', why: 'danger', group: dangerGroup(d), v3: d.kind === 'machine' ? 'dangerous_machine' : 'caught_in_hazard', label: `${d.name} endangers someone`, who: d.name, weights: { '5-7': 3, '8-10': 3 }, cancel: PHYSICAL, moment: `${d.name} is used on, or endangers, a character` }]);
  }
  const items = [];
  const seen = new Set();
  let questions = 0;
  for (const key of ['villain', 'threat', 'child', 'danger', 'changes', 'frightening']) {
...
const nameTokens = (c) => {
  ...
  for (const n of [c.name, ...(c.aliases ?? [])]) {
    const clean = String(n).replace(/^the\s+/i, '').replace(/["“”()]/g, ' ').trim();
    if (!clean) continue;
    toks.push(clean, ...clean.split(/\s+/).filter((t) => t.length >= 3 && !/^(mr|mrs|ms|dr|the|and)\.?$/i.test(t)));
  }
  return [...new Set(toks)];
};

/** Cast members whose name or alias occurs in the scene's verified summary or its lines. Empty when none. */
export function castForScene(cast, summary, lineTexts) {
  const hay = `${summary}\n${lineTexts.join('\n')}`;
  return (cast ?? []).filter((c) => nameTokens(c).some((t) => new RegExp(`\\b${escapeRe(t)}\\b`, 'i').test(hay)));
}
