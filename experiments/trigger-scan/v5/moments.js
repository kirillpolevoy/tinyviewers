#!/usr/bin/env node
// v5 moment finder: where inside a FLAGGED scene the flagged thing happens, so a parent skips the
...
// unsure); end = the latest such line. Span = [begin start - pad, end end + pad] clamped to the scene.
...
/** True when a cue is only a sound caption / speaker tag, with no spoken words. */
export function isCaptionOnly(text) {
  const spoken = String(text)
    .replace(/\([^)]*\)|\[[^\]]*\]/g, ' ') // (door creaking) [music]
    .replace(/^[\s-]*[A-Z][A-Z .'’]+:\s*/g, ' ') // SPEAKER:
    .replace(/[-–—:♪#*]/g, ' ');
 ...
 * cuesById: Map 'L<n>' -> cue; scene: {start_ms, end_ms, start_cue, end_cue}.
 */
/**
 * Span from a begin line index and an end line index. Begin side: back to the end of the previous
 * cue or by pad_back_ms, whichever reaches earlier (policy v5.1; pad_ms when pad_back_ms is unset);
 * end side: pad_ms. The scene's first / last line extend to the scene bounds. Clamped to the scene.
 */
export function spanFromLines(bi, ei, lineIds, cuesById, scene, cfg) {
  const m = cfg.moments;
  const first = cuesById.get(lineIds[bi]);
  const last = cuesById.get(lineIds[ei]);
  let back = first.startMs - (m.pad_back_ms ?? m.pad_ms);
  if (m.pad_back_to_prev_cue && bi > 0) back = Math.min(back, cuesById.get(lineIds[bi - 1]).endMs);
  const start = bi === 0 ? scene.start_ms : Math.max(scene.start_ms, back);
  const stop = ei === lineIds.length - 1 ? scene.end_ms : Math.min(scene.end_ms, last.endMs + m.pad_ms);
  return { start_ms: start, end_ms: stop };
}

export function spanFor({ begin, end, exists }, lineIds, cuesById, scene, cfg) {
  const m = cfg.moments;
  ...
  const { start_ms: start, end_ms: stop } = spanFromLines(bi, ei, lineIds, cuesById, scene, cfg);
  return {
    ...info,
    ...
    span: { start_ms: start, end_ms: stop },
  };
}

/** Merge overlapping or touching spans (gap <= gapMs). */
export function unionSpans(spans, gapMs = 1000) {
  const s = spans.filter(Boolean).map((x) => ({ ...x })).sort((a, b) => a.start_ms - b.start_ms);
  const out = [];
  for (const x of s) {
    const last = out[out.length - 1];
    if (last && x.start_ms <= last.end_ms + gapMs) last.end_ms = Math.max(last.end_ms, x.end_ms);
    else out.push(x);
  }
  return out;
}
...
export function planMoments({ film, scene, cues, reasons, items, cfg }) {
  const m = cfg.moments;
  const whole = (why) => ({ scene: scene.id, call: false, method: 'whole_scene', why, spans: [{ start_ms: scene.start_ms, end_ms: scene.end_ms }] });
...
export function spansFromAnswers(plan, answers, cues, scene, cfg) {
  const cuesById = new Map(cues.map((c) => [`L${c.index}`, c]));
  const perClause = plan.clauses.map((clause, k) => ({
    clause,
    ...spanFor({ begin: answers[`begin.${k}`], end: answers[`end.${k}`], exists: answers[`exists.${k}`]?.noul ?? 0 }, plan.lineIds, cuesById, scene, cfg),
  }));
  const fallback = perClause.filter((c) => !c.span);
  if (fallback.length) {
    return { method: 'whole_scene', why: fallback.map((c) => `${c.fallback}: ${c.clause}`).join('; '), spans: [{ start_ms: scene.start_ms, end_ms: scene.end_ms }], per_reason: perClause };
  }
...
export function respanScenes({ tags, saved, cues, items, cfg }) {
  ...
  for (const s of tags.scenes.filter((x) => x.flagged)) {
    const whole = (why, extra = {}) => ({ method: 'whole_scene', why, spans: [{ start_ms: s.start_ms, end_ms: s.end_ms }], reasons: s.flag_reasons.map((r) => r.id), ...extra });
    ...
    const perReason = rows.map((r) => {
      ...
      return { ...r, span_asrun: r.span, span: spanFromLines(Math.min(bi, ei), Math.max(bi, ei), lineIds, cuesById, s, cfg) };
    });
...
export async function run({ slug, runId = 'r1', cap = 0.02, concurrency = MAX_CONCURRENCY, dryRun = false, post, policyFile }) {
  ...
  const tags = JSON.parse(fs.readFileSync(path.join(here, 'out', `${slug}.tags.${runId}.json`), 'utf8'));
  const jevRun = JSON.parse(fs.readFileSync(path.join(here, tags.jev_file), 'utf8'));
  const seg = JSON.parse(fs.readFileSync(path.join(here, jevRun.segments_file), 'utf8'));
  ...
  const plans = flagged.map((s) => ({ s, plan: planMoments({ film: seg.film, scene: s, cues: cues.slice(s.start_cue - 1, s.end_cue), reasons: s.flag_reasons, items, cfg }) }));
  ...
  for (const { s, plan } of plans) {
    ...
    if (!res?.ok) { scenes[s.id] = { method: 'whole_scene', why: res?.skipped ? 'cap' : `error: ${res?.error}`, spans: [{ start_ms: s.start_ms, end_ms: s.end_ms }], reasons: plan.reasons, request: res?.record ?? null }; continue; }
    const cueSlice = cues.slice(s.start_cue - 1, s.end_cue);
    scenes[s.id] = { ...spansFromAnswers(plan, res.json.answers, cueSlice, s, cfg), reasons: plan.reasons, dialogue_per_min: plan.dialogue_per_min, request: res.record };
  }
  for (const [id, m] of Object.entries(scenes)) {
  ...
  const out = {
    film: seg.film, run: runId, model: MODEL, tags_file: `out/${slug}.tags.${runId}.json`, policy: cfg.moments,
  ...
  const file = path.join(here, 'out', `${slug}.moments.${runId}.json`);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (argv.includes('--respan')) {
    ...
    const from = path.resolve(here, opt('from', `out/${slug}.moments.${runId}.json`));
    ...
    const scenes = respanScenes({ tags, saved, cues, items: tags.film_items ? jevRun.film_items.filter((it) => tags.film_items.some((x) => x.id === it.id)) : jevRun.film_items ?? [], cfg });
    const unflagged = Object.entries(saved.scenes).filter(([id]) => !scenes[id]).map(([id, m]) => ({ id, method: m.method, request: m.request ?? null }));
    const out = {
      ...saved, policy: cfg.moments, scene_bounds: cfg.scene_bounds ?? null,
