#!/usr/bin/env node
// v5 step 1: split the WHOLE film into all its scenes, from VERIFIED SOURCES ONLY, with a cited
// neutral summary per scene, a cited cast and the film's recurring dangers.
//
...
const PROMPT_VERSIONS = { 'v5.0': 'segment-v5.0-verified-sources', 'v5.1': 'segment-v5.1-citation-discipline' };
...
if (HELD_OUT.has(slug) && !flag('final-held-out-run')) { console.error(`${slug} is held out; not segmenting it (pass --final-held-out-run only for the frozen final run).`); process.exit(2); }
const CAP = Number(opt('cap', '0.45'));
const EFFORT = opt('effort', 'medium');
const PROMPT = opt('prompt', 'v5.1');
...
// ---- sources ----------------------------------------------------------------------------------------
const srcFile = path.join(V5, 'sources', `${slug}.json`);
if (!fs.existsSync(srcFile)) throw new Error(`no ${path.relative(V5, srcFile)}; run node sources.js ${slug} first`);
...
- is_child: "true" if the sources show they are a child or young (e.g. called a kid, a son, a toddler), "false" if they show an adult, else "unknown".
- looks_frightening: "true" only if the sources describe their appearance as frightening (e.g. a sound caption or line about fangs, a character screaming at the sight of them, "a monster"), "false" if the sources describe them as harmless-looking, else "unknown".
- disposition: villain (works against the heroes throughout), threat (dangerous in some scenes but not a plotting villain, e.g. a predator), ally, neutral, changes (turns from one to another), or unknown. disposition_note: at most 12 words, the cited reason, e.g. "plans to kidnap children for their screams". Empty when unknown.
...
- cites: for each of name, kind, is_child, looks_frightening, disposition, the ids that support that field. An empty list for a field makes that field "unknown".

2. dangers: things other than characters that recur or matter as dangers in this film: machines, places, objects, groups of creatures, situations (e.g. "a net", "the drop-off", "a storm"). For each: name, kind (machine, place, object, creature_group, situation), note (at most 12 words, what the sources say it does), group (objects_hazards, peril, captivity, injury, creatures_figures, violence, death, eerie, or none), cites. Only dangers the sources mention.

3. scenes: EVERY scene of the film, in order, from the first line to the last.
- A scene is a continuous stretch in one place and situation, the unit a viewer would call a scene. A new scene starts when the film cuts to a different place, a different group of characters, or jumps in time. Typical features have 40 to 150 scenes. Do not merge unrelated action into one scene to save space. Do not split in the middle of an exchange.
...
- setting: at most 6 words, where it happens, only if the sources show it; else "unknown". setting_cites: the ids.
- sentences: 1 to 4 short sentences (each at most 25 words) saying who is there and what happens, each with its own cites.
  ...
  - A scene whose lines are only songs or sound captions still gets a sentence, e.g. "A song plays." citing its lines.${PROMPT === 'v5.1' ? `
...
- Keep each sentence to what the cited text shows: who does or says what, to whom. Leave out adjectives, emotions, colours, sizes, motives and extra details (such as "distraught", "white", "huge", "along with krill") unless a cited line or W-sentence states them.
...
- kind: a talking fish, bird, toy-sized or young animal is "animal" (use is_child "true" for a young one); "child" is only for a human child; "creature" is only for monsters and fantasy beings, never for real animal species.
- setting: give it only when a cited line or W-sentence names or shows the place; else "unknown".` : ''}`;
...
const SCHEMA = obj({
  cast: {
    type: 'array',
    items: obj({
      name: { type: 'string' }, tmdb: { type: 'string' }, aliases: { type: 'array', items: { type: 'string' } },
...
try {
  ...
  console.error(`FAILED: ${err.message}. spent $${wallet.spent.toFixed(4)} -> ${path.relative(V5, errFile)}`);
  process.exit(1);
}
...
data.cast.forEach((raw, i) => {
  ...
  m.aliases = m.aliases.map((a) => clean(a, 8, id, 'notes_trimmed'));
