...
const ITEMS = [
  ...
  { entity: 'C01', name: 'Vera', id: 'C01_threatens', type: 'threatens', group: 'peril', label: 'Vera threatens someone', weights: { '5-7': 3, '8-10': 3 }, cancel: { retold: true, imagined: true }, moment: 'Vera threatens, chases, or attacks someone' },
...
test('imagined cancels physical events and film threats; comic cancels only threats/yelling', () => {
  ...
  const comic = selectScene(answers({ 'e.threatens_harm': 0.9, 'e.attacked': 0.9, 's.laughs': 3 }), cfg);
  assert.deepEqual(comic.cancelled.map((c) => c.id), ['threatens_harm']);
  assert.deepEqual(reasons(comic), ['attacked']);
});
...
test('film-specific: threat / child in danger flag; villain presence alone is a tag', () => {
  ...
  assert.ok(threat.flag_reasons.every((x) => x.film_specific && x.group === 'peril'));
...
test('policy v5.1: dangerous_machine needs caught_in_hazard or the summary channel; reanimated_dead needs danger', () => {
  const captions = selectScene(answers({ 'pl.dangerous_machine': 0.9, 's.danger': 3 }), cfg);
  assert.equal(captions.flagged, false);
  assert.ok(tag(captions, 'dangerous_machine'));
  assert.deepEqual(reasons(selectScene(answers({ 'pl.dangerous_machine': 0.9, 'e.caught_in_hazard': 0.8, 's.danger': 3 }), cfg)).sort(), ['caught_in_hazard', 'dangerous_machine']);
  assert.deepEqual(reasons(selectScene(answers({ 'ps.dangerous_machine': 0.9, 's.danger': 3 }), cfg)), ['dangerous_machine']);
  assert.equal(selectScene(answers({ 'pl.reanimated_dead': 0.9, 's.danger': 1 }), cfg).flagged, false);
  assert.deepEqual(reasons(selectScene(answers({ 'pl.reanimated_dead': 0.9, 's.danger': 3 }), cfg)), ['reanimated_dead']);
});

test('policy v5.1: threatens items generated from a changes arc are ignored; scenes cover the gaps', () => {
  const items = [...ITEMS, { entity: 'C03', name: 'Gil', id: 'C03_threatens', type: 'threatens', why: 'changes', group: 'peril', label: 'Gil threatens someone', cancel: { retold: true, imagined: true }, moment: 'Gil threatens' }];
  const a = answers({ 'fe.C03_threatens': 0.9, 's.danger': 3 });
  const run = { film: { slug: 'x' }, run: 'r1', film_items: items, scenes: [
    { id: 'S001', start_cue: 1, end_cue: 2, start_ms: 5000, end_ms: 20000, answers: a },
    { id: 'S002', start_cue: 3, end_cue: 4, start_ms: 30000, end_ms: 40000, answers: answers() },
  ] };
  const out = selectRun(run, cfg);
  assert.deepEqual(out.film_items_dropped_by_policy.map((x) => x.id), ['C03_threatens']);
  assert.equal(out.scenes[0].flagged, false);
