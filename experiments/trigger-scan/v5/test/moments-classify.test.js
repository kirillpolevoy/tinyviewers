...
test('spanFor: earliest strong begin, latest strong end, padded and clamped; fallbacks', () => {
  ...
  // v5.1 begin side: back to the previous cue's end (12000) or 15 s (5000), whichever is earlier, clamped to the scene
  assert.deepEqual(r.span, { start_ms: scene.start_ms, end_ms: 42000 + 5000 });
  const wide = { ...scene, start_ms: 0 };
  assert.deepEqual(spanFor({ begin: choice({ L12: 1 }), end: choice({ L12: 1 }), exists: 0.9 }, lineIds, cuesById, wide, cfg).span, { start_ms: 30000 - 15000, end_ms: 32000 + 5000 });
  ...
  assert.equal(spanFor({ begin: choice({ L12: 1 }), end: choice({ L12: 1 }), exists: 0.9 }, lineIds, gapCues, wide, cfg).span.start_ms, 12500); // a wordless gap before the line is kept
...
test('spansFromAnswers: any reason not found in the lines means the whole scene', () => {
  ...
  assert.equal(both.spans.length, 1); // 15-37 s and 35-47 s overlap
  const miss = spansFromAnswers(plan, { ...ans, 'exists.1': { noul: 0.1 } }, cues, scene, cfg);
  assert.equal(miss.method, 'whole_scene');
  assert.deepEqual(miss.spans, [{ start_ms: scene.start_ms, end_ms: scene.end_ms }]);
});
...
test('respanScenes: offline re-span keeps only the current reasons, falls back when a scene or reason was never asked', async () => {
  ...
  assert.deepEqual(out.S001.spans, [{ start_ms: 30000 - 15000, end_ms: 32000 + 5000 }]);
