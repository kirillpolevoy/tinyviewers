#!/usr/bin/env node
// v5 selection: pure code over Jev's raw answers (classify.js) and, when present, the moment finder's
...
import { PRESENCE, EVENTS, SCORES, GROUPS, ITEMS, DERIVED } from './questions.js';
...
/**
 * The modifiers that may cancel a film-specific item. policy.film_specific.cancel_exempt lists, per
 * item type, modifiers that never cancel it (a threat is spoken in the present even inside a scene
 * that also retells the past).
 */
export function filmCancel(it, cfg) {
  const exempt = cfg.film_specific?.cancel_exempt?.[it.type] ?? [];
  if (!it.cancel || !exempt.length) return it.cancel;
  return Object.fromEntries(Object.entries(it.cancel).filter(([m]) => !exempt.includes(m)));
}

/**
 * Film items select.js uses. policy.film_specific.drop_generated drops items of a type generated for
 * a given reason (e.g. a 'threatens' question generated from a 'changes' character arc: an ally who
 * changes is not a threat). Their answers are ignored.
 */
export function usableFilmItems(items, cfg) {
  const drop = cfg.film_specific?.drop_generated ?? {};
  return items.filter((it) => !(drop[it.type] ?? []).includes(it.why));
}
...
export function flagRule(tag, answers, tags, cfg) {
...
export function selectScene(answers, cfg, { items = [], context = new Set() } = {}) {
  ...
  for (const t of base.tags) {
    const rule = flagRule(t, answers, base.tags, cfg);
    if (!rule) continue;
...
/** Skip spans: the moment finder's union when it ran for this scene, else the whole scene. */
export function skipFor(scene, flagged, moments) {
  ...
  return { method: m?.method ?? 'whole_scene', spans: [{ start_ms: scene.start_ms, end_ms: scene.end_ms }], ms: scene.end_ms - scene.start_ms };
}
...
/**
 * Scene time bounds. A segment ends at its last line, so the wordless time between scenes belonged
 * to no scene and could never be skipped (Lion King 14.8 min). With policy.scene_bounds, each scene
 * runs to the next scene's start and the first starts at 0. line_start_ms / line_end_ms keep the
 * dialogue bounds.
 */
...
export function selectRun(run, cfg, { moments = null } = {}) {
  const allItems = run.film_items ?? [];
  const items = usableFilmItems(allItems, cfg);
  const droppedItems = allItems.filter((it) => !items.includes(it)).map(({ id, name, type, why }) => ({ id, name, type, why }));
  const rows = run.scenes;
  ...
  const scenes = rows.map((row, i) => {
    ...
    return { ...baseRow, ...r, skip: skipFor(baseRow, r.flagged, moments?.scenes) };
  });
  ...
  return {
    ...
    film_items_dropped_by_policy: droppedItems,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const momFile = path.join(here, 'out', `${slug}.moments.${runId}.json`);
  ...
  const out = selectRun(run, loadPolicy(polFile), { moments });
  ...
  console.log(file);
  console.log(`  flagged ${s.flagged}/${s.scenes} scenes; flagged scenes ${min(s.flagged_scene_ms)} min, skip ${min(s.skip_ms)} min (${out.moments_file ? 'moments' : 'whole scenes'})`);
  ...
  for (const sc of out.scenes) {
    ...
    const act = sc.tags.filter((t) => t.level === 'act').map((t) => `${t.id}${t.layer === 'presence' || t.type === 'presence' ? `[${t.source}]` : ''}:${t.p}${t.film_level ? '(ctx)' : ''}`);
    ...
    console.log(`        act: ${act.join(' ') || '-'}${sc.cancelled.length ? ` | cancelled: ${sc.cancelled.map((c) => `${c.id}(${c.by})`).join(',')}` : ''}${sc.vetoed.length ? ` | kind-gate: ${sc.vetoed.map((v) => `${v.id}->${v.kind}`).join(',')}` : ''} | mods: retold ${sc.modifiers.retold.on ? 'ON' : 'off'} imagined ${sc.modifiers.imagined.on ? 'ON' : 'off'} comic ${sc.modifiers.comic.laughs}${sc.modifiers.comic.on ? ' ON' : ''} | kind ${sc.kind?.choice}:${sc.kind?.confidence}`);
  }
}
