...
import { freezeStatus, HELD_OUT } from '../env.js';
...
test('held-out gate: held-out films need --final-held-out-run and an intact out/freeze.json', () => {
  assert.deepEqual([...HELD_OUT].sort(), ['frankenweenie', 'wild-robot']);
  ...
  assert.equal(run('wild-robot', []).status, 2);
  const st = freezeStatus();
  const r = run('frankenweenie', ['--final-held-out-run']);
