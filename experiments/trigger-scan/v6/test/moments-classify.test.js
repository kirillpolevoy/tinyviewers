...
test('spanFor: earliest strong begin, latest strong end, padded and clamped; fallbacks', () => {
  ...
  // v6: a plain line pads back 15 s (v5.1 ran back over the whole gap, unbounded)
  assert.equal(spanFor({ begin: choice({ L12: 1 }), end: choice({ L12: 1 }), exists: 0.9 }, lineIds, gapCues, wide, cfg).span.start_ms, 25000);
...
test('spansFromAnswers: any reason not found in the lines means the whole scene', () => {
  ...
  assert.deepEqual(miss.spans.map((x) => [x.start_ms, x.end_ms]), [[scene.start_ms, scene.end_ms]]);
});
