...
import { V6, key, filmMeta, HELD_OUT, heldOutGate } from './env.js';
...
async function main() {
  ...
  fs.mkdirSync(path.join(V6, 'sources'), { recursive: true });
  const file = path.join(V6, 'sources', `${slug}.json`);
  fs.writeFileSync(file, JSON.stringify(out, null, 2));
  console.log(`${slug}: TMDB ${tmdb.id} "${tmdb.title}" cast ${tmdb.cast.length}/${tmdb.cast_total}; Wikipedia "${wikipedia.title}" rev ${wikipedia.revision_id} (${wikipedia.verified_by}), section "${wikipedia.section}", ${wikipedia.sentence_count} sentences, ${wikipedia.word_count} words -> ${path.relative(V6, file)}`);
}
