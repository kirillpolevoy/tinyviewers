...
import { V6, TS } from './env.js';
import { statusOf } from './claims.js';

const V5OUT = path.join(TS, 'v5', 'out');
const FILMS = ['nemo', 'monsters-inc', 'lion-king'];
const labels = JSON.parse(fs.readFileSync(path.join(V6, 'calibration', 'verifier-labels.json'), 'utf8'));
...
fs.writeFileSync(path.join(V6, 'calibration', 'claim-rule.json'), JSON.stringify(out, null, 2));
...
if (process.argv.includes('--write') && chosen) {
  const pf = path.join(V6, 'policy.json');
