...
export function spanFromLines(bi, ei, lineIds, cuesById, scene, cfg, cueByIndex = null) {
  ...
  const b = beginMs(first, prev, bi === 0 ? scene.start_ms : floor, cfg);
  const pad = cfg.moments?.pad_ms ?? 5000;
  const stop = ei === lineIds.length - 1
    ? Math.min(scene.end_ms, Math.max(last.endMs + pad, last.endMs + (w.trail_max_ms ?? Infinity)))
    : Math.min(scene.end_ms, last.endMs + pad);
  return { start_ms: Math.min(b.start_ms, first.startMs), end_ms: Math.max(stop, last.endMs), lead_in: b.lead_in };
}
...
export function unionSpans(spans, gapMs = 1000) {
  const s = spans.filter(Boolean).map((x) => ({ start_ms: x.start_ms, end_ms: x.end_ms, ...(x.lead_in ? { lead_in: true } : {}) })).sort((a, b) => a.start_ms - b.start_ms);
  const out = [];
  for (const x of s) {
    const last = out[out.length - 1];
    if (last && x.start_ms <= last.end_ms + gapMs) { last.end_ms = Math.max(last.end_ms, x.end_ms); if (x.lead_in) last.lead_in = true; } else out.push(x);
  }
  return out;
}
