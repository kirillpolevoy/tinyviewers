...
const labels = JSON.parse(fs.readFileSync(path.join(V7, 'calibration', 'verifier-labels.json'), 'utf8'));
...
fs.writeFileSync(path.join(V7, 'calibration', 'claim-rule.json'), JSON.stringify(out, null, 2));
...
if (process.argv.includes('--write') && chosen) {
  const pf = path.join(V7, 'policy.json');
