...
import { key, outDir, heldOutGate, sourcesFile, V7, TS } from './env.js';
...
export const loadGateCfg = (file = path.join(here, 'policy.json')) => JSON.parse(fs.readFileSync(file, 'utf8')).split_gate;
...
export async function checkSegmentation({ film, modelScenes, cues, ctx, cfg, wallet, post, log, skipJev = false }) {
  const { metrics: code, covered } = codeMetrics(modelScenes, cues, ctx);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (reuse) {
    ...
    if (prev.raw_file !== path.relative(V7, path.resolve(rawFile))) throw new Error(`--reuse ${reuse} checked ${prev.raw_file}, not ${rawFile}`);
    // no Jev calls: the saved Jev verdicts; code metrics and the gate recomputed with the current code and policy
    const { metrics: codeM, covered } = codeMetrics(modelScenesOf(raw), cues, { wCount: SRC.wikipedia.sentences.length, tCount: SRC.tmdb.cast.length });
    ...
    if (wallet.spent > 0) record(slug, { script: 'check-split.js', kind: 'jev', usd: wallet.spent, note: `${SPLIT_VERSION} ${label} on ${path.relative(V7, path.resolve(rawFile))}` });
  }
  const file = path.join(outDir(), `${slug}.splitcheck.${label}.json`);
  fs.writeFileSync(file, JSON.stringify({ film, label, raw_file: path.relative(V7, path.resolve(rawFile)), checked_at: new Date().toISOString(), ...(reuse ? { jev_reused_from: path.relative(V7, path.resolve(reuse)) } : {}), gate_cfg: cfg, ...check }, null, 2));
  ...
  if (fold) {
    ...
    const n = foldIntoSegments(seg, check, { label, raw_file: path.relative(V7, path.resolve(rawFile)), checked_at: new Date().toISOString() });
    fs.writeFileSync(path.resolve(fold), JSON.stringify(seg, null, 2));
    console.log(`  folded into ${path.relative(V7, path.resolve(fold))} (${n} scenes)`);
  }
