...
import { V7, key, filmMeta, HELD_OUT, heldOutGate } from './env.js';
...
async function main() {
  ...
  fs.mkdirSync(path.join(V7, 'sources'), { recursive: true });
  const file = path.join(V7, 'sources', `${slug}.json`);
  fs.writeFileSync(file, JSON.stringify(out, null, 2));
  console.log(`${slug}: TMDB ${tmdb.id} "${tmdb.title}" cast ${tmdb.cast.length}/${tmdb.cast_total}; Wikipedia "${wikipedia.title}" rev ${wikipedia.revision_id} (${wikipedia.verified_by}), section "${wikipedia.section}", ${wikipedia.sentence_count} sentences, ${wikipedia.word_count} words -> ${path.relative(V7, file)}`);
}
