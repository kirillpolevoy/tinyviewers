 ...
 */
export function planScene({ seg, scene, cues, items, split = null }) {
  const ctx = contextState({ seg, scene, cues });
  // v9: only the presence / event questions split.json assigns to Jev (split = null: all, as v8)
  const only = split ? { presenceIds: split.jev_presence, eventIds: split.jev_events } : {};
  const linesQ = { ...buildQuestions({ channels: ['pl', 'm'], ...only }), ...filmQuestions(items, { channels: ['fpl'] }) };
  const ctxChannels = ctx.summaryEmpty ? ['e', 'mod', 's', 'kind'] : ['ps', 'e', 'mod', 's', 'kind'];
  const ctxQ = { ...buildQuestions({ channels: ctxChannels, ...only }), ...filmQuestions(items, { channels: ctx.summaryEmpty ? ['fe'] : ['fps', 'fe'] }) };
  const reqs = [
...
export async function run({ slug, runId = 'r1', cap = 0.25, concurrency = MAX_CONCURRENCY, limit, scenesWanted, dryRun = false, post, segFile }) {
  ...
  const universal = buildQuestions({ presenceIds: split.jev_presence, eventIds: split.jev_events });
  ...
  const plans = scenes.map((scene) => ({ scene, ...planScene({ seg, scene, cues: cues.slice(scene.start_cue - 1, scene.end_cue), items, split }) }));
  ...
  const header = {
    ...
    split: { file: 'split.json', sha256_12: split.sha256_12, jev_presence: split.jev_presence.length, jev_events: split.jev_events.length, sonnet_used: split.sonnet_used },
