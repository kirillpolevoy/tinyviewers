 ...
 */
// v9: `presenceIds` / `eventIds` restrict the pl/ps presence questions and the e.* events to the ones
// split.json assigns to Jev (null = all, v8 behaviour). Mention (m.*) questions are not part of the
// split and are always asked for every presence item that has one.
export function buildQuestions({ channels = ['pl', 'ps', 'm', 'e', 'mod', 's', 'kind'], presenceIds = null, eventIds = null } = {}) {
  const want = new Set(channels);
  const pOk = (id) => !presenceIds || presenceIds.includes(id);
  const q = {};
  for (const p of PRESENCE) {
    if (want.has('pl') && pOk(p.id)) q[`pl.${p.id}`] = { type: 'noul', instructions: p.pl ?? `Do \`scene.lines\` show that ${p.noun} is in this scene?`, criteria: { true: join(PL_YES, p.def), false: join(PL_NO, p.noExtra, p.plNoExtra) } };
    if (want.has('ps') && pOk(p.id)) q[`ps.${p.id}`] = { type: 'noul', instructions: p.ps ?? `Does \`scene.summary\` say that ${p.noun} is in this scene?`, criteria: { true: join(PS_YES, p.def), false: join(PS_NO, p.noExtra) } };
  ...
  if (want.has('e')) for (const e of EVENTS) if (!eventIds || eventIds.includes(e.id)) q[`e.${e.id}`] = { type: 'noul', instructions: e.q, ...(e.no || e.yes ? { criteria: { ...(e.yes ? { true: e.yes } : {}), ...(e.no ? { false: e.no } : {}) } } : {}) };
