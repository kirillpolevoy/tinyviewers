#!/usr/bin/env node
// v9 STAGE RUNNER: one film through the whole v9 pipeline, in order, each stage a separate process with
...
//   segment    segment.js         Sonnet split from verified sources + JEV SPLIT GATE      Sonnet <= $0.45, Jev <= $0.03
...
//   select1    select.js          merge (merge.js) + the user's flag policy                (no model)
...
//   checkdesc  check-describe.js  JEV claim check of every sentence; code rebuilds the text Jev <= $0.02
...
export const STAGES = [
  ...
  ['checkdesc', 'check-describe.js', ['--run', 'r1', '--cap', '0.02']],
