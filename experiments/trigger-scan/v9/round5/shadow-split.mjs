#!/usr/bin/env node
// ROUND 5 copy of split/stage-b.mjs: the 24 SHADOW questions (Jev keeps them; Sonnet answers them unused) re-measured on the three held-out films, Jev vs Sonnet on identical v9 scenes, human keys. Evaluation only.
...
const V8OUT = path.resolve(here, '..', 'out'); // ROUND 5: held-out Jev answers + scenes are v9's own (trimmed classify)
const V9OUT = path.resolve(here, '..', 'out');
const argv = process.argv.slice(2);
const opt = (k, d) => { const i = argv.indexOf(`--${k}`); return i >= 0 ? argv[i + 1] : d; };
const ALL = ['book-of-life', 'princess-and-the-frog', 'moana'];
...
const ASKED = split.shadow; // ROUND 5: only the shadow questions have BOTH a Jev and a Sonnet answer on held-out films
...
fs.writeFileSync(path.join(here, 'out', 'shadow-split.json'), JSON.stringify(out, null, 2));
