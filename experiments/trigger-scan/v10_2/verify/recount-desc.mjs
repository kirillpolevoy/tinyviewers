#!/usr/bin/env node
// VERIFIER: recount the blind-judge verdicts in round8/out/descriptions.json (the judge itself is not re-run: no model
...
const d = rj(path.join(V, 'round8', 'out', 'descriptions.json'));
const K = ['accurate', 'partly', 'wrong', 'generic'];
const out = { per_film: {}, pooled: {} }; const all = [];
for (const f of d.per_film) {
  const slug = f.slug; const tags = rj(path.join(V, 'out102', `${slug}.tags.r1.json`)); const built = rj(path.join(V, 'round8', 'live', `${slug}.built.json`));
