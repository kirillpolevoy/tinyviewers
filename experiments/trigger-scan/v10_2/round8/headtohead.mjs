...
const FILMS = (process.argv[2] ?? 'incredibles,big-hero-6,brave').split(',');
const BASE = path.join(V8, 'round8', 'live');
const R5OUT = path.join(V8, 'round8', 'out');
const TAGS_DIR = process.env.R8_TAGS_DIR ? path.resolve(process.env.R8_TAGS_DIR) : null;
...
function whyCheck(whyRun, flagged) {
  const slugW = whyRun.film?.slug ?? whyRun.film; const desc = rj(path.join(outFor(slugW), `${slugW}.describe.r1.json`));
  const out = { flagged: flagged.length, described_only: 0, no_verified_text: 0, stated: 0, generated: 0, unrelated_only: 0, nothing: 0, described: 0, described_plus_reason: 0, plain_reason: 0, missing: [], shown_sentences: 0, shown_titles_sonnet: 0, uncited_or_unverified: [], mismatched: [] };
  for (const s of flagged) {
    ...
    if (!w?.text) { if (w?.source === 'no_verified_text') out.no_verified_text++; else { out.nothing++; out.missing.push(s.id); } continue; }
    ...
    if (w.source === 'described' || w.source === 'described_only' ? rest !== '' : !/^Flagged because [^.]+\.$/.test(rest)) out.uncited_or_unverified.push(`${s.id}:extra-text`);
...
fs.writeFileSync(path.join(R5OUT, process.env.R8_NAME ?? 'headtohead.json'), JSON.stringify(result, null, 2));
