...
test('select (v10): a Jev id tags from its combined answer at its own threshold; a Sonnet id from Sonnet; both flag under the v9 policy', () => {
  const pol = structuredClone(loadPolicy()); delete pol.flag.gating; // v10.2: the policy layer without the tier-A gate (test/v102-gate.test.js)
  const cfg = withJevThresholds(pol);
