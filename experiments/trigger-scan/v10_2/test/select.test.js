...
// v10.2: these tests pin the per-scene policy layer (v6-v10.1 rules); the tier-A gate on top is tested in
// test/v102-gate.test.js, so the gate is taken off here.
const ungated = (p) => { const c = structuredClone(p); delete c.flag.gating; return c; };
const cfg = ungated(loadPolicy());
