...
export const TEXTSAFE_VERSION = 'textsafe-v10.2.0';
...
// ---- v10.2 -------------------------------------------------------------------------------------------------
/**
 * v10.2 fix 2: may this flag reason be WRITTEN as a code-built reason? 'a child cries' only when Jev answered that
 * the one who cries is a child (select.js reason.child 'jev_child'); every other reason as before.
 */
export const reasonSayable = (reason) => reason.id !== 'crying' || reason.child === 'jev_child';

/**
 * v10.2 fix 4, the PLOT PATH for a title or sentence citing a Wikipedia plot sentence (check-describe.js):
 *   status      its lines-based claim status: never 'contradicted'
 *   wsupport    Jev's claim check against ONLY its cited plot sentences: the text margin (accept)
 *   wAllowed    every cited W-sentence is one describe.js gave the scene (anchors / verified cites / order window)
 *   neighbour   fill.js's neighbour test: p(neither) >= neighbourMin (the lines around the scene do not show it)
 *   dirOk       the direction check is not reversed
 * Returns { ok, failed: [...] }.
 */
export function plotPathOk({ status, wsupport, wAllowed, neighbour, dirOk }, { accept, neighbourMin }) {
  const failed = [];
  if (status === 'contradicted') failed.push('lines_contradict');
  if (!wsupport || !textAccepted(wsupport, accept)) failed.push('plot_support');
  if (!wAllowed) failed.push('plot_not_for_scene');
  if (!neighbour || !(Number(neighbour.p_neither) >= neighbourMin)) failed.push('neighbour');
  if (!dirOk) failed.push('direction');
  return { ok: failed.length === 0, failed };
}

/**
 * v10.2: is a stated reason PLACED in the scene? claims.js's placement Choice (fits / conflicts / cannot_tell) of the
 * reason sentence over the scene's own lines: fits >= min_fits and conflicts < max_conflicts (v8's rule for a claim
 * with no lines of its own). Round-7 style false reasons were events of the scene before ('a pet dies' over the
 * mourning scene) or of another character; the claim check alone passed them.
 */
export function reasonPlaced(pl, cfg) {
  const fits = Number(pl?.probabilities?.fits) || 0; const conf = Number(pl?.probabilities?.conflicts) || 0;
  return fits >= cfg.min_fits && conf < cfg.max_conflicts;
}
