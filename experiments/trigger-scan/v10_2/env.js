...
export const DEV_FILMS = ['nemo', 'monsters-inc', 'lion-king', 'frankenweenie', 'wild-robot', 'iron-giant', 'up', 'tangled', 'coco', 'how-to-train-your-dragon', 'book-of-life', 'princess-and-the-frog', 'moana', 'good-dinosaur', 'frozen', 'zootopia'];
...
export function outDir() {
  ...
  const d = process.env.V102_OUT ? path.resolve(V8, process.env.V102_OUT) : path.join(V8, 'out102');
...
export function heldOutGate(slug, argv = process.argv) {
  ...
  if (!st.ok) {
    console.error(`${slug} is held out and ${st.missing ? 'out102/freeze.json does not exist (run node freeze.js first)' : `these frozen files changed since ${st.frozen_at}: ${st.changed.join(', ')}`}. Refusing.`);
    process.exit(2);
  }
}
