...
const v101Freeze = path.join(V8, '..', 'v10_1', 'out101', 'freeze.json');
...
fs.writeFileSync(file, JSON.stringify({ frozen_at: new Date().toISOString(), version: 'v10.2', parent_freeze: parent, jev_set: JEV_SET_VERSION, jev_set_fix_mode: FIX_MODE, allow_proxy: allowProxy, decisions_sha256_12: dsha, dev_films: DEV_FILMS, note: 'sha256 of every file that decides what a model is asked, how answers become sentences, flags, skip spans, a split verdict or a why, or which films may run. Held-out films are refused unless all still match.', files }, null, 2));
