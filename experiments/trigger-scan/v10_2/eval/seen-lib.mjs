...
export const V102 = path.resolve(here, '..');
export const TS = path.resolve(V102, '..');
export const V101 = path.join(TS, 'v10_1');
export const V10 = path.join(TS, 'v10');
export const FILMS = [...FILMS13, 'good-dinosaur', 'frozen', 'zootopia'];
...
export async function liveFor(slug) {
  const files = [path.join(V10, 'round6', 'live', `${slug}.built.json`), path.join(TS, 'v9', 'round5', 'live', `${slug}.built.json`), path.join(TS, 'v8', 'baseline', 'out', `${slug}.built.json`)];
...
export const resolveFile = (slug) => path.join(V102, 'out102', 'dev', `${slug}.resolve.r1.json`);
 ...
 */
export function runSystem(f, S, cfg, { used, gate = null, resolve = null, split = null } = {}) {
  const tags = S.selectRun({ film: { slug: f.slug }, run: 'seen', film_items: f.items, scenes: f.rows }, cfg, { cues: f.cues, sonnet: f.sonnet, used, childcry: f.childcry, ...(resolve ? { resolve } : {}), ...(split ? { split } : {}) });
  ...
  const sp = respanScenes({ tags, saved: f.saved, cues: f.cues, items: f.items, cfg });
  const flagged = tags.scenes.filter((s) => s.flagged);
  for (const s of flagged) s.skip = { method: sp[s.id]?.method, spans: sp[s.id]?.spans ?? [], ms: sp[s.id]?.skip_ms ?? 0 };
