// Paths, keys and the held-out gate shared by every v8 script (copied from v7).
//
...
//
// OUTPUT DIRECTORY: every script reads and writes under outDir() = $V8_OUT (relative to v8/) or
// v8/out. Git-ignored.
//
...
export const DEV_FILMS = ['nemo', 'monsters-inc', 'lion-king', 'frankenweenie', 'wild-robot', 'iron-giant', 'up'];
...
export const sourcesFile = (slug) => path.join(process.env.V8_SOURCES ? path.resolve(V8, process.env.V8_SOURCES) : path.join(V8, 'sources'), `${slug}.json`);

/** The output directory (created on demand). */
export function outDir() {
  const d = process.env.V8_OUT ? path.resolve(V8, process.env.V8_OUT) : path.join(V8, 'out');
...
export const FREEZE_FILE = () => path.join(V8, 'out', 'freeze.json');
