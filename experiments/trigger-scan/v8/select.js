...
export function selectRun(run, cfg, { moments = null, cues = null, reasons = null } = {}) {
  ...
  const scenes = rows.map((row, i) => {
    ...
    // v8 fix (c): why a flagged scene is flagged, for parents (reasons.js); only when the reasons were
    // computed for the same flag reasons
    const w = r.flagged ? reasons?.scenes?.[row.id] : null;
    const why = w && w.reasons.join() === r.flag_reasons.map((x) => x.id).join() ? w.why : null;
    return { ...baseRow, ...r, skip: skipFor(baseRow, r.flagged, moments?.scenes), ...(r.flagged ? { why } : {}) };
  });
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const reaFile = path.join(OUT, `${slug}.reasons.${runId}.json`);
  const reasons = fs.existsSync(reaFile) ? JSON.parse(fs.readFileSync(reaFile, 'utf8')) : null;
  ...
  const out = selectRun(run, loadPolicy(polFile), { moments, cues, reasons });
  out.jev_file = path.relative(here, jevFile);
  out.moments_file = moments ? path.relative(here, momFile) : null;
  out.reasons_file = reasons ? path.relative(here, reaFile) : null;
  const fl = out.scenes.filter((x) => x.flagged);
  out.summary.why = { from_sentence: fl.filter((x) => x.why?.source === 'verified_sentence').length, generated: fl.filter((x) => x.why?.source === 'generated').length, missing: fl.filter((x) => !x.why).map((x) => x.id) };
  ...
  console.log(file);
  console.log(`  flagged ${s.flagged}/${s.scenes} scenes; flagged scenes ${min(s.flagged_scene_ms)} min, skip ${min(s.skip_ms)} min (${out.moments_file ? 'moments' : 'whole scenes'}); why: ${s.why.from_sentence} from a verified sentence, ${s.why.generated} generated, missing ${s.why.missing.length ? s.why.missing.join(',') : 0}`);
