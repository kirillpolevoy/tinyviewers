...
const decisions = path.join(V8, 'assemble', 'decisions.json');
const dsha = crypto.createHash('sha256').update(fs.readFileSync(decisions)).digest('hex').slice(0, 12);
if (dsha !== DECISIONS_SHA256_12) { console.error(`cannot freeze: jev-set.js was built from decisions ${DECISIONS_SHA256_12}, assemble/decisions.json is now ${dsha} (run node assemble/build.mjs)`); process.exit(2); }
...
fs.writeFileSync(file, JSON.stringify({ frozen_at: new Date().toISOString(), version: 'v10', jev_set: JEV_SET_VERSION, jev_set_fix_mode: FIX_MODE, allow_proxy: allowProxy, decisions_sha256_12: dsha, dev_films: DEV_FILMS, note: 'sha256 of every file that decides what a model is asked, how answers become sentences, flags, skip spans, a split verdict or a why, or which films may run. Held-out films are refused unless all still match.', files }, null, 2));
