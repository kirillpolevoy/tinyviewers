...
import { buildFixes } from '../assemble/fixrun/fixes.mjs';
...
const POOL_FILE = path.join(V10, 'jevfirst', 'tournament', 'pool.json');
...
test('every concept expression and threshold equals the decision it was built from (assemble/decisions.json)', () => {
  const D = JSON.parse(fs.readFileSync(path.join(V10, 'assemble', 'decisions.json'), 'utf8'));
