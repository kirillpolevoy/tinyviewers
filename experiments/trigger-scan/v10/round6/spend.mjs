...
//   live  round5/live/<slug>.spend.json    (round6/run-live.mjs; one entry per priced response)
...
export const HELD = ['frozen', 'zootopia', 'good-dinosaur'];
export const SCORED = (process.env.R6_SCORED ?? 'frozen,zootopia,good-dinosaur').split(',');
...
export function spent() {
  const films = {};
  for (const s of HELD) {
    const v9f = path.join(V9, 'out10', `${s}.spend.json`); const lf = path.join(LIVE_OUT, `${s}.spend.json`);
