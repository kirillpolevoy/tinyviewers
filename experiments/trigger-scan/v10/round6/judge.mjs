...
export function build(slug) {
  const t9 = rj(path.join(V9, 'out10', `${slug}.tags.r1.json`));
  const seg = rj(path.join(V9, 'out10', `${slug}.segments.json`));
  ...
  for (const s of t9.scenes.filter((x) => x.flagged && x.why)) {
    ...
    if (s.why.title) items.push({ sys: 'v10', kind: 'title', scene: s.id, skey: `v10:${s.id}`, text: s.why.title, title_source: s.why.title_source });
  }
...
async function judge(slug) {
  ...
  const r = await runCodex(`r6-describe-${slug}`, prompt, schema);
  ...
  return { slug, cached: !!r.cached, commands: r.commands ?? [], items: rows.length, v10_text: tally((x) => x.sys === 'v10' && x.kind === 'text'), v10_title: tally((x) => x.sys === 'v10' && x.kind === 'title'), live_text: tally((x) => x.sys === 'live' && x.kind === 'text'), live_title: tally((x) => x.sys === 'live' && x.kind === 'title'), rows };
}
...
const out = { generated_at: new Date().toISOString(), judge: 'codex exec --ignore-user-config -m gpt-6-astra -c model_reasoning_effort="high" --sandbox read-only --ephemeral (round6/judge.mjs; prompt identical to eval/judge-descriptions.mjs)', films: results.map((r) => r.slug), pooled: { v10_text: pool('v10_text'), live_text: pool('live_text'), v10_title: pool('v10_title'), live_title: pool('live_title') }, v10_text_by_source: bySource, per_film: results };
