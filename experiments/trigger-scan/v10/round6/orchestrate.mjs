#!/usr/bin/env node
// Round-6 (v10 held-out: frozen, zootopia, good-dinosaur; copy of round5/orchestrate.mjs, v10 in place of v9) orchestrator (evaluation harness; not pipeline, not frozen). Runs, for each held-out film,
...
//   node round5/orchestrate.mjs [--films a,b] [--only live|v10]
...
for (const slug of FILMS) {
  if (ONLY !== 'v10') jobs.push(chain(slug, 'live').then((ok) => [slug, 'live', ok]));
  if (ONLY !== 'live') jobs.push(chain(slug, 'v10').then((ok) => [slug, 'v10', ok]));
}
