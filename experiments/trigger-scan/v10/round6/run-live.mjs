...
function bank(slug, stage, usd) {
  ...
  l.entries.push({ at: new Date().toISOString(), script: `round6/run-live.mjs ${stage}`, kind: 'sonnet', usd: +usd.toFixed(6) });
  l.total = +l.entries.reduce((a, e) => a + e.usd, 0).toFixed(6);
  wj(f, l);
}

const [slug, stage = 'all'] = process.argv.slice(2);
if (!HELD.includes(slug) || !['scenes', 'presence', 'build', 'all'].includes(stage)) { console.error(`usage: node round6/run-live.mjs <${HELD.join('|')}> [scenes|presence|build|all]`); process.exit(2); }
