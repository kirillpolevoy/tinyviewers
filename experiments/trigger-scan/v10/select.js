...
export function selectRun(run, cfg0, { moments = null, cues = null, reasons = null, why = null, sonnet = null, used = [] } = {}) {
  ...
  // split.json assigns to Sonnet (merge.js)
  const usedSet = new Set(used);
  const rows = run.scenes.map((row) => {
    if (!row.answers) return row;
    const a = withConcepts(row.answers, items);
    return { ...row, answers: used.length ? mergeAnswers(a, sonnet?.scenes?.[row.id] ?? null, used) : a };
  });
