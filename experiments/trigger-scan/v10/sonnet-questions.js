// v10 SONNET QUESTION LAYER (draft, not frozen). Sonnet is the FALLBACK: it is asked ONLY the v9 ids of the
// concepts where Jev had no reliable phrasing and Sonnet was measurably better on the 13 seen films
// (assemble/decide.mjs tier S; split.json sonnet_asked == sonnet_used, no shadow set). SONNET_OWNED below is
// that list, checked against split.json at import. The wording is still v9's (it is what Sonnet was
// measured on), and so are the prompt, the schema and the probability map.
...
export const SONNET_Q_VERSION = 'sonnetq-v10.0-draft';

/** The v9 ids Sonnet answers in v10, by concept (assemble/decide.mjs tier S). */
export const SONNET_OWNED = {
  seriously_ill: ['seriously_ill'],
  vehicle_crash: ['vehicle_crash'], // vehicle_accident stays with Jev: Sonnet was never measured on it
  captured: ['cage_net_trap', 'restraints', 'captured'],
  graveyard_funeral: ['graveyard_funeral'],
  weapon_used: ['weapon_used'],
  trapped: ['trapped'],
  abandoned: ['abandoned'],
  caregiver_cruelty: ['caregiver_cruelty'],
  crying: ['crying'],
  despair: ['despair'],
};
export const SONNET_OWNED_IDS = Object.values(SONNET_OWNED).flat();
...
// v10 self-check: split.json gives Sonnet exactly SONNET_OWNED_IDS (and asks nothing else)
{
  const fs = await import('node:fs');
  const path = await import('node:path');
  const { fileURLToPath } = await import('node:url');
  const sp = JSON.parse(fs.readFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), 'split.json'), 'utf8'));
  const want = [...SONNET_OWNED_IDS].sort().join();
  if ([...sp.sonnet_used].sort().join() !== want || [...sp.sonnet_asked].sort().join() !== want) throw new Error(`sonnet-questions.js: SONNET_OWNED (${want}) disagrees with split.json (used ${sp.sonnet_used.join(',')}; asked ${sp.sonnet_asked.join(',')})`);
  checkIds(SONNET_OWNED_IDS);
}
