#!/usr/bin/env node
// v10.3 (fix d): --attempt 2 runs the SAME checks on describe2.js's second attempt (<slug>.describe2 -> <slug>.why2);
// describe2.js --merge then builds what parents see (<slug>.whyfinal). Nothing else changed.
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const slug = argv.find((a) => !a.startsWith('--') && !['--run', '--cap', '--in', '--reuse-why', '--attempt'].includes(argv[argv.indexOf(a) - 1]));
  ...
  // v10.3 fix (d): --attempt 2 checks describe2.js's second attempt (<slug>.describe2) and writes <slug>.why2
  const ATTEMPT = opt('attempt', '1');
  // --attempt 3: the title pass (describe2.js --titles, <slug>.describe3 -> <slug>.why3)
  if (!['1', '2', '3'].includes(ATTEMPT)) { console.error('--attempt must be 1, 2 or 3'); process.exit(2); }
  const DNAME = { 1: 'describe', 2: 'describe2', 3: 'describe3' }[ATTEMPT]; const WNAME = { 1: 'why', 2: 'why2', 3: 'why3' }[ATTEMPT];
  const desc = JSON.parse(fs.readFileSync(path.join(IN, `${slug}.${DNAME}.${runId}.json`), 'utf8'));
  ...
  out.attempt = Number(ATTEMPT);
  fs.writeFileSync(path.join(OUT, `${slug}.${WNAME}.${runId}.json`), JSON.stringify(out, null, 2));
