#!/usr/bin/env node
// ROUND 9 (fresh films kung-fu-panda, onward, croods): copy of v10_2/round8/run-live.mjs; only the ledger script name and usage
// line changed (spend guard and output dir come from ./spend.mjs = round9/live). Same relative depth, same scene-api files.
// ROUND 8 (fresh films incredibles, big-hero-6, brave): copy of v10/round6/run-live.mjs; only the spend guard (./spend.mjs), the output directory (round8/live), the ledger script name and the usage line changed. Same relative depth, so the scene-api imports are the same files.
...
//   node round6/run-live.mjs <slug> [scenes|presence|build|all]
...
import { reserve, FILMS as HELD, LIVE_OUT as OUT } from './spend.mjs';
...
function bank(slug, stage, usd) {
  ...
  l.entries.push({ at: new Date().toISOString(), script: `round9/run-live.mjs ${stage}`, kind: 'sonnet', usd: +usd.toFixed(6) });
  l.total = +l.entries.reduce((a, e) => a + e.usd, 0).toFixed(6);
  wj(f, l);
}

const [slug, stage = 'all'] = process.argv.slice(2);
if (!HELD.includes(slug) || !['scenes', 'presence', 'build', 'all'].includes(stage)) { console.error(`usage: node round9/run-live.mjs <${HELD.join('|')}> [scenes|presence|build|all]`); process.exit(2); }
