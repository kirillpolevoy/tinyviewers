#!/usr/bin/env node
// ROUND 9 (v10.3, fresh films kung-fu-panda, onward, croods): copy of v10_2/round8/judge.mjs. SAME PROMPT TEXT, labels,
// flags and judge; changes: films/paths from round9/spend.mjs (v10.3 out103 -- the shown text is tags.why, which select3
// takes from the merged whyfinal.r1.json; live round9/live), seed / cache r9-describe-<slug>, output
// round9/out/descriptions.json. v10.3 has no code-built reasons (policy text_safety.code_built_reasons false), so no
// 'reason' notes are built unless a 'Flagged because' line appears (it would be sent and counted, as in round 8).
...
async function judge(slug) {
  ...
  const r = await runCodex(`r9-describe-${slug}`, prompt, schema);
...
const out = { generated_at: new Date().toISOString(), false_code_built: ok.flatMap((r) => r.false_code_built.map((x) => ({ slug: r.slug, ...x }))), v10_reason_pooled: pool('v10_reason'), v10_plain_title_pooled: pool('v10_plain_title'), no_text: Object.fromEntries(ok.map((r) => [r.slug, r.no_text])), judge: 'codex exec --ignore-user-config -m gpt-6-astra -c model_reasoning_effort="high" --sandbox read-only --ephemeral (round9/judge.mjs; prompt identical to v10_2/round8/judge.mjs, v10_1/round7/judge.mjs, round6/judge.mjs and eval/judge-descriptions.mjs)', films: results.map((r) => r.slug), pooled: { v10_text: pool('v10_text'), live_text: pool('live_text'), v10_title: pool('v10_title'), live_title: pool('live_title') }, v10_text_by_source: bySource, per_film: results };
