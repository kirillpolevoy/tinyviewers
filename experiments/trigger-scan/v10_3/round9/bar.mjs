#!/usr/bin/env node
// ROUND 9 BAR (v10.3 vs live on the fresh films kung-fu-panda, onward, croods; evaluation only, no model calls). Copy of
// v10_2/round8/bar.mjs with round 9's PRE-REGISTERED BAR (pooled, all must hold), written before any fresh output was read:
//   1 recall      conservative recall >= live and the jitter verdict is not 'live'
...
//   5 text        judged-accurate share of shown v10.3 texts >= live's, 0 uncited / unverified shown text or title
//                 (headtohead whyCheck), 0 FALSE TEXT. FALSE TEXT (v10.3's own definition, eval/seen.mjs): shown text
//                 that is not Jev-verified or cites nothing, plus code-built statements (none should exist). The
//                 blind judge's 'wrong' verdicts on shown v10.3 texts / titles are reported next to it as the STRICT
//                 reading (false text = judged wrong); the verdict is given under both readings.
//   6 no-text     share of flagged scenes with no shown text <= 0.25
//   7 placeholder share of flagged scenes with the 'Flagged scene' title <= 0.25
//   8 quote       0 quote-rule violations (corrected tokenizer, round9/quote.mjs) in v10.3's pipeline outputs
// reasons-truth.json (code-built reason truth judge) is not used: v10.3 builds no reason text.
...
const d = rj('descriptions.json'); const truth = { per_film: [] }; const h = rj('headtohead.json'); const w = rj('who-flags.json'); const q = rj('quote.json');
...
function descFor(films) {
  ...
  return {
    ...
    wrong_v103_text_count: v.filter((x) => x.kind === 'text' && x.verdict === 'wrong').length, wrong_v103_title_count: v.filter((x) => x.kind === 'title' && x.verdict === 'wrong').length,
  };
}
...
const codeBuilt = h.films.flatMap((f) => (f.parent.v10.iii_why.code_built ?? []).map((x) => `${f.slug}:${x}`));
const quotePipe = q.v103_and_eval.model_text_hits.filter((x) => x.path.startsWith('out103/'));
...
const bar = {
  ...
  text: { accurate_share_v103: desc.v102_accurate_share, accurate_share_live: desc.live_accurate_share, uncited: uncited.length, uncited_list: uncited, false_text: uncited.length + codeBuilt.length, code_built: codeBuilt, judged_wrong_v103_texts: desc.wrong_v103_text_count, judged_wrong_v103_titles: desc.wrong_v103_title_count, pass: desc.v102_accurate_share >= desc.live_accurate_share && uncited.length === 0 && codeBuilt.length === 0, pass_strict_reading: desc.v102_accurate_share >= desc.live_accurate_share && uncited.length === 0 && codeBuilt.length === 0 && desc.wrong_v103_text_count + desc.wrong_v103_title_count === 0 },
  no_text: { v103: desc.no_text, share: desc.no_text_share, pass: desc.no_text_share <= 0.25 },
  placeholder_title: { v103: desc.placeholder_title, share: desc.placeholder_title_share, pass: desc.placeholder_title_share <= 0.25 },
  quote: { v103_pipeline_violations: quotePipe.length, list: quotePipe, eval_notes_violations: q.v103_and_eval.model_text_hits.length - quotePipe.length, live_violations: q.live_pipeline_outputs.model_text_hits.length, old_tokenizer_v103_hits: q.v103_and_eval.old_rule_hits.length, pass: quotePipe.length === 0 },
};
const CRIT = ['recall', 'precision', 'skip_minutes', 'wordless_peaks', 'text', 'no_text', 'placeholder_title', 'quote'];
bar.all_pass = CRIT.every((k) => bar[k].pass);
bar.all_pass_strict_text_reading = bar.all_pass && bar.text.pass_strict_reading;
bar.failed = CRIT.filter((k) => !bar[k].pass);
...
const b = bar;
console.log(`== POOLED ${films.join('+')}: ALL_PASS ${b.all_pass} (strict text reading ${b.all_pass_strict_text_reading})`);
...
console.log(`  text ${JSON.stringify(desc.v102_text)} (${desc.v102_accurate_share}) live ${JSON.stringify(desc.live_text)} (${desc.live_accurate_share}); uncited ${b.text.uncited}; false text ${b.text.false_text}; judged wrong v103 texts ${b.text.judged_wrong_v103_texts} titles ${b.text.judged_wrong_v103_titles}; v103 titles ${JSON.stringify(desc.v102_title)} (${desc.v102_title_accurate_share}) live titles ${JSON.stringify(desc.live_title)} ${b.text.pass} (strict ${b.text.pass_strict_reading})`);
console.log(`  no text ${desc.no_text} (${desc.no_text_share}) ${b.no_text.pass}; placeholder ${desc.placeholder_title} (${desc.placeholder_title_share}) ${b.placeholder_title.pass}; quote ${JSON.stringify({ ...b.quote, list: undefined })}; FAILED ${JSON.stringify(b.failed)}`);
