// Question set v10 (draft, not frozen) = v9's v6 question set + the v10 JEV SET.
//
// v10: the per-scene concept questions Jev answers are no longer built here from PRESENCE / EVENTS; they
// are the tournament-chosen phrasings in jev-set.js (generated by assemble/build.mjs from
// assemble/decisions.json), each asked in its own MINIMAL state (statesFor below: lines only, lines
// without lyrics, summary only, lines + summary, optionally with the verified children list, or v9's
// context state for the v9 wording), and combined in code (combine.js) into one probability per v9 id
// with a per-concept threshold (jevThresholds). What stays exactly as v9 asked it: the mention questions
// (m.*), the modifiers (retold, imagined), the Scores, the kind Choice and film presence (fpl / fps).
// PRESENCE / EVENTS below are still the item catalogue (ids, groups, labels, weights, moment clauses,
// cancel sets) and the v9 wording Sonnet is asked (sonnet-questions.js). Everything v6 says below holds
// for those parts.
//
...
import { QUESTIONS as JEV_QUESTIONS, CONCEPTS as JEV_CONCEPTS, JEV_SET_VERSION, FIX_MODE as JEV_FIX_MODE } from './jev-set.js';
import { evalExpr, keysOf } from './combine.js';

export const VERSION = 'v10.0-draft';
...
// v10.1 (user policy (3)): believed_dead ('Thought dead') is cancelled by the COMIC modifier (laughs >= comic.on_at):
// a death faked as a prank or played for laughs (Good Dinosaur S008, Buck's prank) is a tag, not a flag. A real
// death (dies, loved_one_dies) is never cancelled by comedy.
...
export const EVENTS = [
  ...
  E('believed_dead', 'believed_dead', 'death', 'Thought dead', 'In `scene`, does a character believe that someone close to them has just died?', '"I\'m dead" as a figure of speech is a no. Remembering a death from long ago is a no.', 3, 3, { cancel: { comic: true }, moment: 'a character believes someone close to them has just died' }),
...
// =============================================================================================
// v10: THE JEV SET (jev-set.js) -- states, film-template instantiation, planning, and the combine step
// =============================================================================================
export { JEV_QUESTIONS, JEV_CONCEPTS, JEV_SET_VERSION, JEV_FIX_MODE };

// Lyric lines (the tournament's rule, jevfirst/tournament/pool.mjs): a cue with a music sign, or the run of
// cues after a (... SINGING ...) caption until the next caption or speaker label (at most 8 cues).
const MUSIC = /[♪♫]|^#|#$/;
...
/** Verified child names (+ verified aliases), for the ...C states. */
...
/**
 * Every state a v10 question may be asked in, for one scene (the tournament's states, identical bodies):
 *   L     { film:{title}, scene:{lines} }                  (= v9's lines state)
 *   Lnl   the same, lyric lines removed
 *   S     { film:{title}, scene:{setting, summary} }       verified summary only; null when it is empty
 *   LS    { film:{title}, scene:{setting, summary, lines} }; LSnl with the lyric lines removed
 *   ...C  the same with { children: [verified child names] } first
 *   V9C   v9's context state (contextState): cast, dangers, setting, summary, lines
 */
export function statesFor({ seg, scene, cues }) {
  ...
  return { states: st, lyricCues: lyr.size, summaryEmpty: !summary, castUsed: ctx.castUsed };
}

/** Film type of every template key (from the film concepts' targets and the `each` nodes that read it). */
export const TEMPLATE_TYPE = (() => {
  const m = {};
  const walk = (e, type) => {
    if (!e || typeof e !== 'object') return;
    if (e.q !== undefined && e.q.includes('{item}')) { if (m[e.q] && m[e.q] !== type) throw new Error(`questions.js: template ${e.q} read for two film types`); m[e.q] = type; }
    for (const k of ['max', 'min']) if (Array.isArray(e[k])) e[k].forEach((x) => walk(x, type));
    if (e.gate) { walk(e.gate, type); walk(e.then, type); }
    if (e.score) walk(e.then, type);
    if (e.each) walk(e.of, e.each.type);
  };
  for (const c of Object.values(JEV_CONCEPTS)) for (const t of c.targets) walk(t.expr, t.layer === 'film' ? t.type : null);
  for (const [k, t] of Object.entries(m)) if (!t) throw new Error(`questions.js: template ${k} has no film type`);
  return m;
})();
...
/** A film template for one generated item: the question carries the name + verified aliases, the criteria the plain name (as the tournament asked it). */
export function instantiateTemplate(q, it, seg) {
  const who = nameWithAliases(it, seg);
  const rep = (s, w) => SLOT_WORDS.reduce((acc, slot) => acc.replaceAll(slot, w), s);
  return { ...q, instructions: rep(q.instructions, who), criteria: { true: rep(q.criteria.true, it.name), false: rep(q.criteria.false, it.name) } };
}

/**
 * v10 Jev questions of one film: [{ key, state, q }], templates instantiated once per generated item of
 * their type ('#{item}' -> '#<item id>').
 */
export function jevQuestions(seg, items) {
  const out = [];
  for (const [key, d] of Object.entries(JEV_QUESTIONS)) {
    if (!key.includes('{item}')) { out.push({ key, state: d.state, q: d.q }); continue; }
    const type = TEMPLATE_TYPE[key];
    for (const it of items.filter((x) => x.type === type)) out.push({ key: key.replaceAll('{item}', it.id), state: d.state, q: instantiateTemplate(d.q, it, seg), item: it.id });
  }
  return out;
}

/** Per-concept thresholds for select.js: 'c.<v9 id>' (presence / event), 'd.<derived id>', 'cf.<film type>'. */
export const JEV_THRESHOLDS = (() => {
  const t = {};
  for (const c of Object.values(JEV_CONCEPTS)) {
    for (const x of c.targets) {
      const k = x.layer === 'film' ? `cf.${x.type}` : x.layer === 'derived' ? `d.${x.id}` : `c.${x.id}`;
      if (t[k] !== undefined && t[k] !== x.act) throw new Error(`questions.js: two thresholds for ${k}`);
      t[k] = x.act;
    }
  }
  return t;
...
/** v9 ids (presence / event) whose answer comes from the v10 Jev set. */
export const JEV_TARGET_IDS = Object.values(JEV_CONCEPTS).flatMap((c) => c.targets.filter((t) => t.layer === 'presence' || t.layer === 'event').map((t) => t.id));

/**
 * The combine step (pure): a scene's raw v10 answers -> the per-id probabilities select.js tags.
 *   raw.q    { key: p } every v10 Jev Noul answered for this scene (film templates with item ids)
 *   raw.s    Scores (the danger Score gates some bundles)
 * Returns { c: { v9 id: p }, d: { derived id: p }, cf: { film item id: p } }.
 */
export function conceptAnswers(raw, items = []) {
  const ctx = { a: raw?.q ?? {}, scores: { danger: Number(raw?.s?.danger?.score ?? 0) }, items };
  const c = {}; const d = {}; const cf = {};
  for (const con of Object.values(JEV_CONCEPTS)) {
...
// self-check: every key a concept reads is a defined question; every template has a film type
for (const con of Object.values(JEV_CONCEPTS)) for (const t of con.targets) for (const k of keysOf(t.expr)) if (!JEV_QUESTIONS[k]) throw new Error(`questions.js: ${k} is read but not in jev-set.js`);
for (const t of JEV_TARGET_IDS) if (!ITEMS[t]) throw new Error(`questions.js: jev-set target ${t} is not an item`);
