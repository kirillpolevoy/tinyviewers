...
import { budget, attemptCaps } from './budget.js';
import { creditsSpan, isCreditsScene } from './credits.js';
...
if (!slug) { console.error('usage: node segment.js <slug> [--cap 1.05] [--attempt1-cap 0.45] [--attempt2-cap 0.60] [--split-cap 0.05] [--effort low] [--attempt1-from <raw.json>] [--no-retry] [--dry]'); process.exit(2); }
// --final-held-out-run: the one-time final run on a held-out film, after all code is frozen.
heldOutGate(slug);
// v10.1: --cap is the per-film Sonnet cap; inside it attempt 1 and attempt 2 have their own caps
// (budget.js attemptCaps: attempt 2's share is set aside first, so attempt 1 cannot starve the retry).
const CAP = Number(opt('cap', '1.05'));
const A1_CAP = Number(opt('attempt1-cap', '0.45'));
const A2_CAP = Number(opt('attempt2-cap', '0.60'));
...
const GATE_CFG = { ...POLICY.split_gate, credits: POLICY.credits }; // v10.1: the end-credits rule travels with the gate
...
if (!fs.existsSync(srcFile)) throw new Error(`no ${path.relative(V8, srcFile)}; run node sources.js ${slug} first`);
...
const CAPS = attemptCaps({ cap: CAP, prior: PRIOR, a1: A1_CAP, a2: A2_CAP });
let wallet = budget(CAPS.a1_cap); // attempt 1's wallet; replaced by attempt 2's before the retry
const spentAll = { usd: 0 }; // Sonnet spend of this run, both attempts
...
async function callPart(first, last, part, { minOutput, maxOut, limit = Infinity }) {
  ...
  // v10.1: `limit` keeps half 1 of attempt 2 to half of the attempt's cap, so half 2 always has room
  const left = Math.min(wallet.cap - wallet.spent - wallet.reserved, limit);
  ...
  console.log(`  ${label}: input ${counted != null ? `${counted} tok counted` : 'not counted'}, reserving ${inWorst}, max_tokens ${maxTokens}, effort ${EFFORT}, reserve $${worst.toFixed(3)} (left $${left.toFixed(3)} of this attempt's cap $${wallet.cap.toFixed(3)}; film cap $${CAP.toFixed(2)}, already spent on this film before this run $${PRIOR.toFixed(4)})`);
  ...
  try {
    ...
    wallet.settle(worst, cost); spentAll.usd += cost;
...
console.log(`${slug}: "${film.title}" (${film.year ?? '?'}), ${N} cues, ${W.length} W, ${T.length} T; sonnet film cap $${CAP.toFixed(2)} (spent before $${PRIOR.toFixed(4)}; attempt 1 cap $${CAPS.a1_cap.toFixed(3)}, attempt 2 cap $${CAPS.a2_cap.toFixed(3)} set aside), split-check cap $${SPLIT_CAP}`);
const attempts = [];
let accepted = null;
...
// ---- attempt 1: whole film (or replayed) ---------------------------------------------------------------
{
  ...
  if (ATTEMPT1_FROM) {
    ...
    a1 = { data: raw.data, usage: raw.usage ?? null, cost: 0, wall_ms: 0, replayed_from: path.relative(V8, path.resolve(ATTEMPT1_FROM)), replayed_run_at: raw.run_at ?? null, range: [1, N], part: null };
    ...
    if (flag('dry') && !ATTEMPT1_FROM) {
      ...
      wallet = budget(CAPS.a2_after(0));
      console.log(`attempt 2 (only if attempt 1 fails the gate): halves L${h.a[0]}-L${h.a[1]} and L${h.b[0]}-L${h.b[1]}, attempt-2 cap $${wallet.cap.toFixed(3)}`);
      for (const [k, [f, l]] of [[1, h.a], [2, h.b]]) await callPart(f, l, { k, of: 2 }, { minOutput: 12_000, maxOut: 32_000, limit: k === 1 ? wallet.cap / 2 : Infinity }).catch((e) => console.log(`  ${e.message}`));
      process.exit(0);
    }
  }
...
if (!accepted && !flag('no-retry')) {
  const h = halves(N, RETRY.overlap_cues);
  wallet = budget(CAPS.a2_after(spentAll.usd)); // attempt 2's own cap (>= the share set aside before attempt 1)
  ...
  try {
    const A = await callPart(h.a[0], h.a[1], { k: 1, of: 2 }, { minOutput: 12_000, maxOut: 32_000, limit: wallet.cap / 2 });
...
if (!accepted) {
  ...
  fs.writeFileSync(errFile, JSON.stringify({ film, run_at: runAt, error: 'segmentation rejected by the split gate on every attempt', attempts: attemptSummary, gate_reports: attempts.map((a) => a.gate ?? null), sonnet_spent_usd: +spentAll.usd.toFixed(5), attempt_caps: { film_cap: CAP, prior: +PRIOR.toFixed(5), attempt1_cap: CAPS.a1_cap, attempt2_cap: CAPS.a2_cap }, split_check_spent_usd: +splitWallet.spent.toFixed(6) }, null, 2));
...
const out = {
  ...
  cap_usd: CAP,
  attempt_caps_usd: { attempt1: CAPS.a1_cap, attempt2: CAPS.a2_cap },
...
// v10.1: the end-credits span (credits.js; informational here -- classify / sonnetq / select recompute it from the SRT)
{
