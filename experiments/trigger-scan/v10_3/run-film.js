#!/usr/bin/env node
// v10.3 STAGE RUNNER: v10.2's + mortal (fix a: the mortal-danger questions, Jev) after resolve, and after checkdesc the
// second describe attempt (fix d): describe2 (Sonnet, only scenes whose first text or title failed), checkdesc2 (the
// same Jev checks on it), titles (Sonnet, a title restating a verified text that still has no verified title),
// checkdesc3 (Jev), mergetext (code: whyfinal), then select3. Outputs in out103 (env.js). select.js applies
// fix (b) (co-occurrence) and fix (c) (span bridge) itself.
...
export const STAGES = [
  ...
  ['mortal', 'mortal.js', ['--run', 'r1', '--cap', '0.04']],
  ...
  ['describe2', 'describe2.js', ['--run', 'r1', '--cap', '0.12']],
  ['checkdesc2', 'check-describe.js', ['--run', 'r1', '--cap', '0.06', '--attempt', '2']],
  ['titles', 'describe2.js', ['--run', 'r1', '--titles', '--cap', '0.04']],
  ['checkdesc3', 'check-describe.js', ['--run', 'r1', '--cap', '0.03', '--attempt', '3']],
  ['mergetext', 'describe2.js', ['--run', 'r1', '--merge']],
