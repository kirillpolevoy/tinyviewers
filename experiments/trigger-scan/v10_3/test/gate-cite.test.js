...
test('held-out gate (v10.3): the 19 seen films are dev (incredibles, big-hero-6, brave since round 8); every other slug needs --final-held-out-run and an intact out103/freeze.json', () => {
  assert.deepEqual([...DEV_FILMS].sort(), ['big-hero-6', 'book-of-life', 'brave', 'coco', 'frankenweenie', 'frozen', 'good-dinosaur', 'how-to-train-your-dragon', 'incredibles', 'iron-giant', 'lion-king', 'moana', 'monsters-inc', 'nemo', 'princess-and-the-frog', 'tangled', 'up', 'wild-robot', 'zootopia']);
  assert.equal(HELD_OUT.has('brave'), false);
  assert.equal(HELD_OUT.has('frozen'), false);
  assert.equal(HELD_OUT.has('zootopia'), false);
