#!/usr/bin/env node
// VERIFIER (round 9, adapted from v10_2/verify/rebuild-live.mjs): re-derive round8/live/<slug>.built.json from the stored live scenes.json + presence.json with the unchanged
...
for (const slug of ['kung-fu-panda', 'onward', 'croods']) {
  const L = path.join(V, 'round9', 'live'); const meta = rj(path.join(TS, 'films.json'))[slug];
