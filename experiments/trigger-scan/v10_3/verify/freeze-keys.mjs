#!/usr/bin/env node
// VERIFIER (round 9, adapted from v10_2/verify/freeze-keys.mjs; paths/films/regexes changed): freeze integrity + ordering, held-out hygiene (v10.2 code/dev/eval never names a fresh film), key blindness
...
const out = {};
// ---- freeze
const fzF = path.join(V, 'out103', 'freeze.json'); const fz = rj(fzF);
...
const fresh = fs.readdirSync(path.join(V, 'out103')).filter((f) => FILMS.some((s) => f.startsWith(`${s}.`))).map((f) => ({ f, m: mt(path.join(V, 'out103', f)) })).sort((a, b) => a.m - b.m);
const logs = fs.existsSync(path.join(V, 'out103', 'logs')) ? fs.readdirSync(path.join(V, 'out103', 'logs')).filter((f) => FILMS.some((s) => f.includes(s))).map((f) => ({ f: `logs/${f}`, m: mt(path.join(V, 'out103', 'logs', f)) })) : [];
...
const parentF = path.join(TS, 'v10_2', 'out102', 'freeze.json');
...
const RE = /kung-fu-panda|kung fu panda|\bonward\b|croods|tai lung|\bshifu\b|barley|manticore|\beep\b|\bgrug\b/i;
const scanDirs = ['dev', 'eval', 'test', 'labels', 'sources', path.join('out103', 'dev'), path.join('out103', 'seen')].map((d) => path.join(V, d));
...
out.keys = {
  ...
  key_text_mentions_pipeline: FILMS.filter((s) => /v10_?3|out103|v10\.3|tags\.r1|round9/i.test(fs.readFileSync(path.join(TS, 'refs', `${s}.key.json`), 'utf8'))),
  refs_code_mentions_pipeline: ['collect.js', 'map.js', 'rules.js'].filter((f) => /out103|v10_3|round9|tags\.r1/.test(fs.readFileSync(path.join(TS, 'refs', f), 'utf8'))),
...
const closure = new Set(); const q = [path.join(V, 'round9', 'run-live.mjs')];
...
out.live = { closure: rel, changed_vs_53dcd9c: diff ? diff.split('\n') : [], dirty: status ? status.split('\n') : [], untracked_in_closure: rel.filter((r) => !tracked.includes(r)),
  closure_mtimes_latest: iso(Math.max(...[...closure].map(mt))), live_outputs: Object.fromEntries(FILMS.map((s) => [s, ['scenes', 'presence', 'built', 'spend'].map((k) => `${k} ${iso(mt(path.join(V, 'round9', 'live', `${s}.${k}.json`)))}`)])) };
